...
public:
	HRESULT	WINAPI Open(D3DXINCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData, UINT *pBytes)
	{
		...
		byte *pData = new byte[uiBufferSize];
		if (!pData)
...
bool	D3D_LoadVertexShader(const tstring &sName, const tstring &sFunction, int &iIndex)
{
	...
	if (g_ShaderCache.LoadShader(sFilename, cBuffer, uiCRC32))
	{
		...
		if (FAILED(D3DXGetShaderConstantTable((DWORD*)pCode->GetBufferPointer(), &g_aVertexShaderSlots[iIndex].pConstantTable)))
		{
	...
	else
	{
...

		{
			...
			if (vid_shaderDebug)
			{
				...
				D3DXCompileShaderFromFile(sSystemPath.c_str(), pMacros, &g_ShaderIncludeSystem, szFuncName, szProfile, dwShaderFlags, &pCode, &pErrorMsgs, &g_aVertexShaderSlots[iIndex].pConstantTable);
			...
			else
			{
				...
				D3DXCompileShader(pBuffer, uiBufferSize, pMacros, &g_ShaderInclude, szFuncName, szProfile, dwShaderFlags, &pCode, &pErrorMsgs, &g_aVertexShaderSlots[iIndex].pConstantTable);
	...
	g_aVertexShaderSlots[iIndex].pFileCallback = new CVertexShaderFileCallback(sFilename, iIndex);
