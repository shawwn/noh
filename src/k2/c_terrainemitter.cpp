...
/*====================
  CTerrainEmitterDef::~CTerrainEmitterDef
  ====================*/
CTerrainEmitterDef::~CTerrainEmitterDef()
{
}
...
/*====================
  CTerrainEmitterDef::Spawn
  ====================*/
IEmitter*   CTerrainEmitterDef::Spawn(uint uiStartTime, CParticleSystem *pParticleSystem, IEmitter *pOwner)
{
    PROFILE("CTerrainEmitterDef::Spawn");

    return K2_NEW(ctx_Effects, CTerrainEmitter)(uiStartTime, pParticleSystem, pOwner, *this);
}


/*====================
  CTerrainEmitter::~CTerrainEmitter
  ====================*/
CTerrainEmitter::~CTerrainEmitter()
{
}
...
/*====================
  CTerrainEmitter::CTerrainEmitter
  ====================*/
CTerrainEmitter::CTerrainEmitter(uint uiStartTime, CParticleSystem *pParticleSystem, IEmitter *pOwner, const CTerrainEmitterDef &eSettings) :
IEmitter
(
    eSettings.GetLife(),
...
m_fDepthBias(eSettings.GetDepthBias())
{
    ...
    if (m_riMaxParticleLife.Max() == -1 || m_rfSpawnRate.Max() == 0.0f)
    {
        ...
        {
            iMaxActive = 1024;
            Console.Warn << m_pParticleSystem->GetEffect()->GetPath() << _T(": <terrainemitter> Infinite spawning emitter with infinite life particles") << newl;
        }
        else
...
/*====================
  CTerrainEmitter::ResumeFromPause
  ====================*/
void    CTerrainEmitter::ResumeFromPause(uint uiMilliseconds)
{
...
/*====================
  CTerrainEmitter::UpdateEmbeddedEmitter
  ====================*/
inline
bool    CTerrainEmitter::UpdateEmbeddedEmitter(uint uiMilliseconds, ParticleTraceFn_t pfnTrace, IEmitter *pEmitter, CSimpleParticle &cParticle)
{
...
/*====================
  CTerrainEmitter::Update
  ====================*/
bool    CTerrainEmitter::Update(uint uiMilliseconds, ParticleTraceFn_t pfnTrace)
{
    PROFILE("CTerrainEmitter::Update");
    ...
    {
        Console.Warn << m_pParticleSystem->GetEffect()->GetPath() << _T(": <terrainemitter> iDeltaTime == ") << iDeltaTime << newl;
        iDeltaTime = 0xffff;
    }
#else
    ...
    float   fRange(m_tfRange.Evaluate(fLerp, fTime));
    ...
    bool bActive(m_bActive && m_pParticleSystem->GetActive() && m_pParticleSystem->GetWorld() != nullptr);
    ...
    if (m_fAccumulator >= 1.0f && (m_iCount == -1 || m_iSpawnCount < m_iCount))
    {
        ...
        while (m_fAccumulator >= 1.0f && (m_iCount == -1 || m_iSpawnCount < m_iCount))
        {
            ...
            CVec3f v3WorldPos(v3LerpedPos);

            CVec2f v2Circle(M_RandomPointInCircle());

            CVec3f v3TerrainPos;
            v3TerrainPos.x = v2Circle.x * fRange + v3WorldPos.x;
            v3TerrainPos.y = v2Circle.y * fRange + v3WorldPos.y;

            if (m_pParticleSystem->GetWorld()->IsInBounds(v3TerrainPos.x, v3TerrainPos.y))
                v3TerrainPos.z = m_pParticleSystem->GetWorld()->GetTerrainHeight(v3TerrainPos.x, v3TerrainPos.y);
            else
            {
                ++m_iSpawnCount;
                continue;
...
#if 0
            if (DistanceSq(v3TerrainPos, v3WorldPos) > SQR(fRange))
            {
                ++m_iSpawnCount;
                continue;
            }
            ...
            CVec3f v3FaceNormal(m_pParticleSystem->GetWorld()->GetTerrainNormal(v3TerrainPos.x, v3TerrainPos.y));
            CVec3f v3Dir;
            if (m_v3Dir != CVec3f(0.0f, 0.0f, 1.0f))
            {
            ...
            CVec3f  v3Position(v3OffsetPos + v3TerrainPos);
    ...
    {
        CVec3f v3BasePos(v3Pos);
        CVec3f v3BaseVelocity((v3BasePos - m_v3LastBasePos) / fDeltaTime);
...
/*====================
  CTerrainEmitter::GetNumBillboards
  ====================*/
uint    CTerrainEmitter::GetNumBillboards()
{
...
/*====================
  CTerrainEmitter::GetBillboard
  ====================*/
bool    CTerrainEmitter::GetBillboard(uint uiIndex, SBillboard &outBillboard)
{
...
/*====================
  CTerrainEmitter::GetNumEmitters
  ====================*/
uint    CTerrainEmitter::GetNumEmitters()
{
...
/*====================
  CTerrainEmitter::GetEmitter
  ====================*/
IEmitter*   CTerrainEmitter::GetEmitter(uint uiIndex)
{
    uint uiParticle((m_uiFrontSlot + uiIndex) % m_vParticles.size());
