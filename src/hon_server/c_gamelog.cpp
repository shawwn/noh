...
void    CGameLog::WritePlayer(EGameLogEvent eEvent, CPlayer *pPlayer, const tstring &sParamA, const tstring &sParamB)
{
    ...
    case GAME_LOG_PLAYER_SWAP:
        if (pPlayer->HasSelectedHero())
            WRITE_STAT(oldhero, QuoteStr(EntityRegistry.LookupName(pPlayer->GetSelectedHero())));
        if (!sParamA.empty())
            WRITE_STAT(newhero, QuoteStr(sParamA));
        break;
...
void    CGameLog::WriteDeny(IUnitEntity *pTarget, IUnitEntity *pAttacker, IGameEntity *pInflictor, float fExperience, ushort unGold)
{
    ...
    if (fExperience >= 0.01f)
        WRITE_STAT(experience, XtoA((float)INT_CEIL(fExperience * 100.0f) / 100.0f, 0, 0, 2));
    if (unGold > 0)
...
/*====================
  CGameLog::WriteAward
  ====================*/
void    CGameLog::WriteAward(EGameLogEvent eEvent, IUnitEntity *pAttacker, IUnitEntity *pTarget, ushort unGold)
{
    if (!m_hLogFile.IsOpen())
        return;
    ...
    if (pOwner != nullptr)
    {
        ...
        else if (eEvent == GAME_LOG_AWARD_HUMILIATION)
            m_hLogFile << SPACE << _T("count:") << pOwner->GetHumiliations();
        else if (eEvent == GAME_LOG_AWARD_RIVAL)
            m_hLogFile << SPACE << _T("count:") << pOwner->GetRivals();
        else if (eEvent == GAME_LOG_AWARD_PAYBACK)
    ...
    if (unGold != 0)
        m_hLogFile << SPACE << _T("gold:") << unGold;
