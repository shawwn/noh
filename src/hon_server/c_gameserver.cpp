...
#include "../hon_shared/c_entitycreepspawner.h"
#include "../hon_shared/i_bitentity.h"
#include "../hon_shared/c_entitypowerupspawner.h"
#include "../hon_shared/c_entitycritterspawner.h"
...
CVAR_UINTF(         sv_unitPingDelay,               SecToMs(2u),    CVAR_GAMECONFIG);
...
CVAR_UINT(          sv_arrangedMatchWaitConnectionTime, SecToMs(20u));
CVAR_UINT(          sv_arrangedMatchWaitTotalTime,      SecToMs(60u));
...
#ifndef K2_CLIENT
CVAR_UINTF(     svr_AccountAuthInterval,        MinToMs(2u),            CVAR_SAVECONFIG);
CVAR_UINTF(     svr_AccountAuthTimeOut,         SecToMs(15u),           CVAR_SAVECONFIG);
#endif
//=============================================================================
...
m_uiLongServerFrame(0)
{
    ...
    m_pServerEntityDirectory = K2_NEW(ctx_GameServer,   CServerEntityDirectory);
}
    ...
    case VOTE_TYPE_CONCEDE:
        {
            ...
            if (GetMatchTime() >= g_voteConcedeUnanimousTime && iVotesRequired > 1)
                --iVotesRequired;
    ...
    case VOTE_TYPE_KICK_AFK:
        {
            ...
            if (!IsKickVoteValid(pTarget) || !pTarget->HasFlags(PLAYER_FLAG_IS_AFK))
            {
                ResetPlayerVotes();
            ...
            if (iYesVotes >= iVotesRequired)
            {
                ...
                m_GameLog.WritePlayer(GAME_LOG_PLAYER_KICKED_AFK, pTarget);
    ...
    case VOTE_TYPE_KICK:
        {
            ...
            // Make sure you can't kick your partner in a 2v0 scenario
            if (iVotesRequired < 2)
                iVotesRequired = 2;
...
void    CGameServer::UpdatePauseStatus()
{
    ...
    else
    {
        ...
        if (uiRemainingTime / 1000 != (uiRemainingTime + GetFrameLength()) / 1000)
        {
            ...
            buffer << GAME_CMD_PAUSE_COUNTDOWN << TStringToUTF8(XtoA((uiRemainingTime / 1000) + 1) + _T("...")) << byte(0);
...
void    CGameServer::Frame()
{
...
#ifndef K2_CLIENT
        if (m_bSentTrialGame == false && Game.GetGameTime() > g_voteRemakeTimeLimit) // After ~5 mins send trial accounts to increase on the server and client.
        {
    ...
    // Check initial connection timeout
    if (m_pHostServer->IsArrangedMatch() &&
        GetGamePhase() == GAME_PHASE_WAITING_FOR_PLAYERS &&
        GetGameTime() > m_uiMatchCreationTime + sv_arrangedMatchWaitConnectionTime &&
        !AllPlayersConnected())
    {
        Reset(false, _T("disconnect_mm_connect_timeout"), true, MATCH_ABORT_CONNECT_TIMEOUT);
    }
}
...
#ifndef K2_CLIENT
/*====================
  CGameServer::GetAccountAuth
  ====================*/
void    CGameServer::GetAccountAuth()
{
    
    if (Host.GetServer()->GetSessionCookie().empty())
        return; 

    if ((Host.GetTime() < m_uiLastAccountAuthRequest + svr_AccountAuthInterval) && m_bSentAccountAuthRequest == false)
        return;
    ...
    if (m_bSentAccountAuthRequest == false)
    {
        ...
        for (PlayerMap_it it(m_mapClients.begin()); it != m_mapClients.end(); ++it)
        {
            ...
            CPlayerAccountStats* pAccountStats(&m_mapPlayerAccountStats[pPlayer->GetAccountID()]);
        ...
        if (bSendAccountAuth)
        {
            m_pAccountAuthRequest->SendPostRequest();
            m_bSentAccountAuthRequest = true;
    ...
    else if(m_pAccountAuthRequest && (!m_pAccountAuthRequest->IsActive() || m_uiSentAccountAuthRequest + svr_AccountAuthTimeOut < Host.GetTime()))
    {
        m_uiLastAccountAuthRequest = Host.GetTime();
        m_bSentAccountAuthRequest = false;

        if (!m_pAccountAuthRequest->WasSuccessful())
        {
        ...
        if (pCookies != NULL)
        {
            ...
            for(int iArrayIndex(0); iArrayIndex < iArraySize; ++iArrayIndex)
            {
                for (PlayerMap_it it(m_mapClients.begin()); it != m_mapClients.end(); ++it)
                {
                    ...
                    if (pAccountStats->GetAccountID() !=  AtoI(pCookies->GetKeyName(iArrayIndex)))//SuperID()
                        continue;

                    tstring sCookie(pCookies->GetVar(iArrayIndex)->GetString());
                    tstring sGameCookie(pGameCookies->GetVar(iArrayIndex)->GetString());

                    if ((pAccountStats->GetGameCookie() != sGameCookie || sGameCookie == TSNULL) && !pPlayer->HasFlags(PLAYER_FLAG_TERMINATED))
                    {
                        Console << _T("Game Cookie failed for ") << pPlayer->GetName() << newl;
                        Console << pAccountStats->GetGameCookie() << newl;
                        Console << sGameCookie << newl;
                        m_pHostServer->KickClient(pPlayer->GetClientNumber(), _T("disconnect_duplicate_game_connect"));
                    ...
                    CClientConnection* pClient(Host.GetServer()->GetClient(pPlayer->GetClientNumber()));
                    if (pClient && (pClient->GetCookie() != sCookie || sCookie == TSNULL))
                    {
                        Console << _T("Cookie failed for ") << pPlayer->GetName() << newl;
                        Console << pClient->GetCookie() << newl;
                        Console << sCookie << newl;
                        m_pHostServer->KickClient(pPlayer->GetClientNumber(), _T("disconnect_duplicate_login"));
                        break;
                    }
                }
            }
...
/*====================
  CGameServer::AllPlayersConnected
  ====================*/
bool    CGameServer::AllPlayersConnected() const
{
    uint uiTeam1Count(0);
    uint uiTeam2Count(0);
...
void    CGameServer::StartMatch()
{
    ...
    for (PlayerMap_it  itClient(m_mapClients.begin()); itClient != m_mapClients.end(); ++itClient)
    {
        CPlayer *pClient(itClient->second);
        
        if (!pClient)
            continue;

        m_pHostServer->GenerateInvitation(pClient->GetAccountID());

#ifndef K2_CLIENT
        SendReconnectData(pClient);
#endif
...
void    CGameServer::EndMatch(int iLosingTeam)
{
    ...
    if (pGameInfo != NULL && !pGameInfo->HasFlags(GAME_FLAG_CONCEDED))
    {
        ...
        // Log passive gold earned by all players
        for (PlayerMap_cit itPlayer(m_mapClients.begin()); itPlayer != m_mapClients.end(); ++itPlayer)
        {
...
void    CGameServer::Remake()
{
    ...
    for (WorldEntList_it it(vWorldEnts.begin()), itEnd(vWorldEnts.end()); it != itEnd; ++it)
    {
        ...
        {
            Precache(pWorldEntity->GetType(), PRECACHE_ALL, TSNULL);
            continue;
        }
        ...
        Precache(pNewEnt->GetType(), PRECACHE_ALL, TSNULL);
    }
...
/*====================
  CGameServer::Reset
  ====================*/
void    CGameServer::Reset(bool bFailed, const tstring &sReason, bool bAborted, EMatchAbortedReason eAbortedReason)
{
    ReplayManager.StopRecording();
    ...
    m_pServerEntityDirectory->Clear();
    m_GameLog.Close();
    m_pHostServer->EndGame(m_sReplayHost, m_sReplayDir, ReplayManager.GetReplayFilename(), m_sGameLogPath, bFailed, sReason, bAborted, eAbortedReason);
    m_mapClients.clear();
...
/*====================
  ...
  ====================*/
void    CGameServer::BalanceTeams(const bool bAutoBalancedMode)
{
    ...
    {
        while ((pCurrentTeam->GetRank() <= pOtherTeam->GetRank() || pOtherTeam->IsFull()) &&
            !deqPlayers.empty() &&
            !pCurrentTeam->IsFull())
        {
    ...
    case GAME_PHASE_WAITING_FOR_PLAYERS:
        {
            ...
            if (GetGameTime() >= GetPhaseEndTime())
            {
                ...
                if (m_pHostServer->IsArrangedMatch())
                {
                    Reset(false, _T("disconnect_mm_start_timeout"), true, MATCH_ABORT_START_TIMEOUT);
                    return false;
                }
            ...
            if (GetGameTime() >= GetPhaseEndTime())
            {
                ...
                if (bStartMatch)
                {
                    ...
                    for (PlayerMap_it it(m_mapClients.begin()); it != m_mapClients.end(); ++it)
                    {
                        ...
                        if (!pPlayer->HasSelectedHero())
                            SelectPotentialHeroOrRandomHero(it->first);
                        if (!pPlayer->HasSelectedAvatar())
                            SelectBaseAvatar(it->first);
                    }
...
void    CGameServer::ParseGameLog(const tstring &sPath)
{
    ...
    while (!hGameLog.IsEOF())
    {
                ...
                case ENTITY_BASE_TYPE_HERO:
                    {
                        ...
                        else if (TStringCompare(sAttacker, _T("Neutral_Kongor")) == 0)
        ...
        case GAME_LOG_AWARD_FIRST_BLOOD:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    CGameStats::SFirstBloodEntry entry(
                        ev.GetPropertyInt(_T("time")),
                        ev.GetPropertyInt(_T("player")),
                        ev.GetPropertyInt(_T("owner")));
                    itStats->second->LogFirstBlood(entry);

                    int iGold = ev.GetPropertyInt(_T("gold"));
                    itStats->second->AddGoldEarned(iGold);
                    itStats->second->AddHeroBounty(iGold);
                    WRITE_TEAM_STAT(GoldEarned, iGold);
                }
            }
            break;

        case GAME_LOG_AWARD_KILL_STREAK:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    uint uiKillStreak(ev.GetPropertyInt(_T("count")));
                    if (itStats->second->GetKillStreak() < uiKillStreak)
                    itStats->second->SetKillStreak(uiKillStreak);
                }
            }
            break;

        case GAME_LOG_AWARD_MULTI_KILL:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    int iMultiKill(ev.GetPropertyInt(_T("count")));
                    switch (iMultiKill)
                    {
                        case 2:
                            WRITE_PLAYER_STAT(MultiKillX2, 1)
                            break;

                        case 3:
                            WRITE_PLAYER_STAT(MultiKillX3, 1)
                            break;

                        case 4:
                            WRITE_PLAYER_STAT(MultiKillX4, 1)
                            break;

                        case 5:
                            WRITE_PLAYER_STAT(MultiKillX5, 1)
                            break;
                    }
                }
            }
        ...
        case GAME_LOG_AWARD_SMACKDOWN:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    CGameStats::SSmackdownEntry entry(
                        ev.GetPropertyInt(_T("time")),
                        ev.GetPropertyInt(_T("player")),
                        ev.GetPropertyInt(_T("owner")));
                    itStats->second->LogSmackdown(entry);
                }
            }
            break;
        case GAME_LOG_AWARD_HUMILIATION:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    CGameStats::SHumiliationEntry entry(
                        ev.GetPropertyInt(_T("time")),
                        ev.GetPropertyInt(_T("player")),
                        ev.GetPropertyInt(_T("owner")));
                    itStats->second->LogHumiliation(entry);
                }
            }
            break;
        case GAME_LOG_AWARD_RIVAL:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    CGameStats::SRivalEntry entry(
                        ev.GetPropertyInt(_T("time")),
                        ev.GetPropertyInt(_T("player")),
                        ev.GetPropertyInt(_T("owner")),
                        ev.GetPropertyInt(_T("count")));
                    itStats->second->LogRival(entry);
                }
            }
            break;
        case GAME_LOG_AWARD_PAYBACK:
            {
                map<int, CGameStats*>::iterator itStats(mapPlayerStats.find(ev.GetPropertyInt(_T("player"), -1)));
                if (itStats != mapPlayerStats.end())
                {
                    CGameStats::SPaybackEntry entry(
                        ev.GetPropertyInt(_T("time")),
                        ev.GetPropertyInt(_T("player")),
                        ev.GetPropertyInt(_T("owner")));
                    itStats->second->LogPayback(entry);
                }
            }
            break;
        }
...
void    CGameServer::SendTrialGameIncrease()
{
    ...
    for (PlayerMap_it itPlayer(m_mapClients.begin()); itPlayer != m_mapClients.end(); ++itPlayer)
    {
        ...
        m_itPlayerAccountStats = m_mapPlayerAccountStats.find(iAccountId);
        int iClientNumber = itPlayer->second->GetClientNumber();

        if (m_itPlayerAccountStats == m_mapPlayerAccountStats.end())
        {
            Console.Err << _T("Server missing account map information!") << newl;
            assert(!"Account Stats find failed");
            continue;
        } 

        if (m_itPlayerAccountStats->second.GetAccountType() == 1 && m_itPlayerAccountStats->second.GetTrialStatus() > 0)
        {
            m_itPlayerAccountStats->second.IncTrialGamesPlayed();
...
/*====================
  CGameServer::SendReconnectData
  ====================*/
#ifndef K2_CLIENT
void    CGameServer::SendReconnectData(CPlayer *pClient)
{
    if (GetGameInfo()->GetMatchID() == -1)
        return;
    ...
    tstring sIP(m_pHostServer->GetAddress());
...
void    CGameServer::SendStats()
{
    ...
    for (PlayerMap_it itPlayer(m_mapClients.begin()); itPlayer != m_mapClients.end(); ++itPlayer)
    {
        ...
        bool bDisconnected(bTerminated && (pPlayer->HasFlags(PLAYER_FLAG_IS_AFK) || !pPlayer->HasFlags(PLAYER_FLAG_KICKED)));
        ...
        pRequest->AddVariable(sPlayer + _T("[discos]"), bDisconnected ? 1 : 0);
        ...
        if (m_pHostServer->IsArrangedMatch())
        {
            // If a match is arranged (whether it's ranked or casual ranked) they are submitted in the same way
            pRequest->AddVariable(sPlayer + _T("[amm_team_rating]"), bWinner ? pPlayer->GetStoredMatchWinValue() : pPlayer->GetStoredMatchLossValue());
            pRequest->AddVariable(sPlayer + _T("[amm_team_count]"), 1);         
        }
        else
        {
        ...
        pRequest->AddVariable(sPlayer + _T("[gold]"), MAX((int)pStats->GetGoldEarned() - (int)pStats->GetGoldLost(), 0));
        ...
        pRequest->AddVariable(sPlayer + _T("[wards]"), pStats->GetWardsPlaced());
        pRequest->AddVariable(sPlayer + _T("[bloodlust]"), pStats->GetFirstBloodEntry().bFirstBlood);
        pRequest->AddVariable(sPlayer + _T("[doublekill]"), pStats->GetMultiKillX2());
        pRequest->AddVariable(sPlayer + _T("[triplekill]"), pStats->GetMultiKillX3());
        pRequest->AddVariable(sPlayer + _T("[quadkill]"), pStats->GetMultiKillX4());
        pRequest->AddVariable(sPlayer + _T("[annihilation]"), pStats->GetMultiKillX5());

        if (pStats->GetKillStreak() >= 3)
            pRequest->AddVariable(sPlayer + _T("[ks3]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks3]"), 0);

        if (pStats->GetKillStreak() >= 4)
            pRequest->AddVariable(sPlayer + _T("[ks4]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks4]"), 0);

        if (pStats->GetKillStreak() >= 5)
            pRequest->AddVariable(sPlayer + _T("[ks5]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks5]"), 0);

        if (pStats->GetKillStreak() >= 6)
            pRequest->AddVariable(sPlayer + _T("[ks6]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks6]"), 0);

        if (pStats->GetKillStreak() >= 7)
            pRequest->AddVariable(sPlayer + _T("[ks7]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks7]"), 0);

        if (pStats->GetKillStreak() >= 8)
            pRequest->AddVariable(sPlayer + _T("[ks8]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks8]"), 0);

        if (pStats->GetKillStreak() >= 9)
            pRequest->AddVariable(sPlayer + _T("[ks9]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks9]"), 0);

        if (pStats->GetKillStreak() >= 10)
            pRequest->AddVariable(sPlayer + _T("[ks10]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks10]"), 0);

        if (pStats->GetKillStreak() >= 15)
            pRequest->AddVariable(sPlayer + _T("[ks15]"), 1);
        else
            pRequest->AddVariable(sPlayer + _T("[ks15]"), 0);

        pRequest->AddVariable(sPlayer + _T("[smackdown]"), (uint)pStats->GetSmackdownLog().size());
        pRequest->AddVariable(sPlayer + _T("[humiliation]"), (uint)pStats->GetHumiliationLog().size());

        pRequest->AddVariable(sPlayer + _T("[nemesis]"), (uint)pStats->GetRivalLogCount());
        pRequest->AddVariable(sPlayer + _T("[retribution]"), (uint)pStats->GetPaybackLog().size());
...
/*====================
  CGameServer::SendAdditionalStats
  ====================*/
void    CGameServer::SendAdditionalStats()
{
    if (GetGameInfo()->GetMatchID() == -1)
        return;
    ...
    pRequest->SetTargetURL(K2System.GetMasterServerAddress() + "/server_requester.php");
    ...
    // Header
    pRequest->AddVariable(_T("f"), _T("set_replay_size"));
    pRequest->AddVariable(_T("session"), m_pHostServer->GetSessionCookie());
    pRequest->AddVariable(_T("match_id"), GetGameInfo()->GetMatchID());
    pRequest->AddVariable(_T("file_size"), uiReplaySize);
...
#ifndef K2_CLIENT
/*====================
  CGameServer::UpdateSubmitStatsRequest
  ====================*/
void    CGameServer::UpdateSubmitStatsRequest()
{
    ...
    if (m_pStatSubmitRequest->WasSuccessful())
    {
        CPHPData phpResponse(m_pStatSubmitRequest->GetResponse());

        Console << _T("Stat submission successful") << newl;

        phpResponse.Print();

        pGameInfo->SetStatsStatus(STATS_SUCCESSFUL);
    }
    else
    {
        CPHPData phpResponse(m_pStatSubmitRequest->GetResponse());

        Console.Err << _T("Stat submission failure") << newl;

        phpResponse.Print();

        pGameInfo->SetStatsStatus(STATS_FAILURE);
    }
...
CPlayer*    CGameServer::TryAddClient(CClientConnection *pClientConnection)
{
    ...
    m_itPlayerAccountStats = m_mapPlayerAccountStats.find(pClientConnection->GetAccountID());
    if (m_itPlayerAccountStats != m_mapPlayerAccountStats.end())
        pPlayer->SetAccountStats(&m_itPlayerAccountStats->second);
    ...
    CBufferDynamic buffer;
    buffer << GAME_CMD_LOBBY_CONNECT_MESSAGE2 << TStringToUTF8(pPlayer->GetName()) << byte(0) << ushort(pPlayer->GetFlags()) << pPlayer->GetAccountIcon() << pPlayer->GetChatSymbol() << pPlayer->GetChatNameColor();
    BroadcastGameData(buffer, true);
...
/*====================
  CGameServer::AddFakePlayer
  ====================*/
CPlayer*    CGameServer::AddFakePlayer(int iTeam)
{
    // Allocate a new player
    IGameEntity* pNewPlayer(m_pServerEntityDirectory->Allocate(Player));
    if (pNewPlayer == NULL)
    ...
    pPlayer->FakeInitialize(m_pHostServer, iClientNum, _T("FakePlayer") + XtoA(iClientNum));

    ChangeTeam(iClientNum, uint(iTeam));
...
bool    CGameServer::IsDuplicateAccountInGame(int iAccountID)
{
    for (PlayerMap_it it(m_mapClients.begin()); it != m_mapClients.end(); ++it)
    {
        ...
        {           
            ...
            // so if this is their first connection (or if they are reconnecting and their old connection hasn't 
            // timed out) it will boot them.
            if (!pPlayer->HasFlags(PLAYER_FLAG_WAS_CONNECTED))
                return true;
        }       
    }
    ...
    try
    {
        ...
        if (!pPlayer->HasFlags(PLAYER_FLAG_KICKED))
        {
            ...
            if (GetGamePhase() == GAME_PHASE_ACTIVE && !pPlayer->HasFlags(PLAYER_FLAG_EXCUSED))
            {
                ...
                if (pHero != NULL && pHero->GetDeathTimeStamp() != INVALID_TIME && sReason != _T("Connection timed out"))
                {
                    ...
                    {
                        CBufferFixed<2> quitBuffer;
                        quitBuffer << GAME_CMD_RAGE_QUIT_MESSAGE2 << byte(pPlayer->GetAnnouncerVoice());
        ...
        if (GetGamePhase() >= GAME_PHASE_HERO_BAN)
        {
            ...
            else
            {
                ...
                m_GameLog.WritePlayer(GAME_LOG_PLAYER_DISCONNECT, pPlayer, sReason);    
            }
        ...
        if (GetGamePhase() <= GAME_PHASE_WAITING_FOR_PLAYERS)
        {
            ...
            if (m_pHostServer->IsArrangedMatch())
                Reset(false, _T("disconnect_mm_player_left"), true, MATCH_ABORT_PLAYER_LEFT);
        }
    }
    catch (CException &ex)
    {
...
{
    if (GetGamePhase() != GAME_PHASE_WAITING_FOR_PLAYERS ||
        (uiTeamID != TEAM_1 && uiTeamID != TEAM_2) ||
        m_pHostServer->IsArrangedMatch())
        return;
    ...
    case VOTE_TYPE_KICK_AFK:
        {
            ...
            if (!IsKickVoteValid(pTarget) || !pPlayer->HasFlags(PLAYER_FLAG_IS_AFK) || pPlayer->GetTeam() != pTarget->GetTeam())
                break;
    ...
    case VOTE_TYPE_KICK:
        {
            ...
            pGameInfo->StartVote(VOTE_TYPE_KICK, iTarget, g_voteDuration, GetTotalTime());

            pTarget->SetLastVoteKickTime(GetGameTime());
        }
        break;
...
bool    CGameServer::ProcessGameData(int iClientNum, CPacket &pkt)
{
    ...
    case GAME_CMD_SELECT_AVATAR:
        SelectAvatar(iClientNum, pkt.ReadTString());
        return !pkt.HasFaults();
        
    case GAME_CMD_SELECT_HERO:
        SelectHero(iClientNum, pkt.ReadShort(-1), false, true);
        return !pkt.HasFaults();

    case GAME_CMD_SELECT_POTENTIAL_HERO:
        SelectHero(iClientNum, pkt.ReadShort(-1), true, false);
        return !pkt.HasFaults();
    ...
    case GAME_CMD_REPICK_HERO:
        if (pPlayer != NULL && pPlayer->CanRepick())
        {
            ...
            pPlayer->SetSelectedAvatarBits(INVALID_ENT_TYPE);
            pPlayer->SetSelectedAvatarKey(INVALID_INDEX);
            if (HasGameOptions(GAME_OPTION_FORCE_RANDOM))
                SelectRandomHero(iClientNum);

            SetHeroStatus(unOldHero, HERO_LIST_AVAILABLE_ALL);
    
            CBufferFixed<7> FullBuffer;
            CBufferFixed<7> EnemyBuffer;
            FullBuffer << GAME_CMD_REPICK_HERO_MESSAGE << iClientNum << unOldHero;
            ...
            {   
                ...
                BroadcastGameDataToTeam(TEAM_SPECTATOR, FullBuffer, true);
                BroadcastGameDataToTeam(iTeam == TEAM_1 ? TEAM_2 : TEAM_1, EnemyBuffer, true);
            }
            else
            {
                BroadcastGameData(FullBuffer, true);
            }
        }
        return true;
    ...
    case GAME_CMD_SWAP_HERO_REQUEST:
        {
            ...
            if (pPlayer->CanSwap() && pTeam != NULL)
            {
                ...
                if (pTargetPlayer != NULL && pTargetPlayer->CanSwap())
                {
                    ...
                    {
                        m_GameLog.WritePlayer(GAME_LOG_PLAYER_SWAP, pPlayer, EntityRegistry.LookupName(pTargetPlayer->GetSelectedHero()));
                        m_GameLog.WritePlayer(GAME_LOG_PLAYER_SWAP, pTargetPlayer, EntityRegistry.LookupName(pPlayer->GetSelectedHero()));
    ...
    case GAME_CMD_UNIT_PING:
        try
        {
            uint uiUnitId(pkt.ReadInt());

            if (pkt.HasFaults())
            {
                Console.Err << "Bad unit ping request" << newl;
                return false;
            }
            ...
            if (pTeam->GetTeamID() == pUnit->GetTeam())
            {
                if (pUnit->IsBuilding() && !pUnit->GetAsBuilding()->GetIsShop() && !pUnit->GetAsBuilding()->GetNoAltClickPing())
                {
                    yPingType = PING_PROTECT_BUILDING;
                }
                else
                    break;
            }
            else if (pPlayer->IsEnemy(pUnit))
            {
                if (pUnit->IsBuilding() && !pUnit->GetAsBuilding()->GetIsShop() && !pUnit->GetAsBuilding()->GetNoAltClickPing())
                {
                    yPingType = PING_ATTACK_BUILDING;
                }
                else if (pUnit->IsHero())
                {
                    yPingType = PING_ATTACK_HERO;
                }
                else
                    break;
            }
            ...
            if (yPingType != -1 && (pPlayer->GetLastUnitPingTime() == INVALID_TIME || GetGameTime() - pPlayer->GetLastUnitPingTime() > sv_unitPingDelay))
            {
                ...
                pPlayer->GetTeamInfo()->SetTeamTarget(pUnit->GetIndex());
            }
        }
        catch (CException &ex)
        {
            ex.Process(_T("CGameServer::ProcessGameData(GAME_CMD_UNIT_PING) - "), NO_THROW);
        }
        break;
...
bool    CGameServer::LoadWorld(const tstring &sName, const tstring &sGameSettings)
{
    ...
    try
    {
        ...
        mapGameSettings[_T("allheroes")] = _T("false");
        ...
        // Add global world modifiers
        const tstring &sWorldModifiers(GetWorldPointer()->GetModifiers());
        if (!sWorldModifiers.empty())
        {
            tsvector vsModifiers(TokenizeString(sWorldModifiers, _T(',')));

            for (tsvector_it it(vsModifiers.begin()), itEnd(vsModifiers.end()); it != itEnd; ++it)
                AddGlobalModifier(EntityRegistry.RegisterModifier(*it));
        }
        ...
        if (m_pHostServer->IsArrangedMatch())
            SetGamePhase(GAME_PHASE_WAITING_FOR_PLAYERS, sv_arrangedMatchWaitTotalTime, m_uiMatchCreationTime);
        else
...
ResHandle   CGameServer::RegisterIcon(const tstring &sPath)
{
    ...
    K2_WITH_GAME_RESOURCE_SCOPE()
    {
        ResHandle hHandle(g_ResourceManager.Register(K2_NEW(ctx_GameServer,   CTexture)(sPath, TEXTURE_2D, TEX_FULL_QUALITY, TEXFMT_A8R8G8B8), RES_TEXTURE, RES_TEXTURE_IGNORE_ALL));
        if (hHandle == INVALID_RESOURCE)
            return INVALID_RESOURCE;
...
{
    EntityRegistry.ServerPrecache(EntityRegistry.LookupID(sName), PRECACHE_ALL, _T("All"));
}
...
bool    CGameServer::SelectHero(int iClientNumber, ushort unHero, bool bPotentialHero, bool bAllowAvatar)
{
    ...
    {
        if (GetGamePhase() < GAME_PHASE_HERO_SELECT || !pPlayer->HasFlags(PLAYER_FLAG_CAN_PICK))
            return false;
    }
    ...
    {   
        if (!bPotentialHero)
        {
            ...
            if (!bAllowAvatar && !pPlayer->HasSelectedAvatar())
            {
            ...
            CBufferFixed<7> FullBuffer;
            CBufferFixed<7> EnemyBuffer;
            FullBuffer << GAME_CMD_PICK_HERO_MESSAGE << iClientNumber << unHero;
...
/*====================
  CGameServer::SelectAvatar
  ====================*/
bool    CGameServer::SelectAvatar(const int iClientNumber, const tstring &sAltAvatar)
{
    ...
    // If they haven't selected a hero yet they can't select an avatar
    if (!pPlayer->HasSelectedHero())
        return false;
            
    // Look up their team info
    int iTeam(pPlayer->GetTeam());
    if (iTeam == 0)
        return false;
    CTeamInfo *pTeam(GetTeam(iTeam));
...
void    CGameServer::SelectRandomHero(int iClientNumber)
{
    ...
    // TODO: Select random available avatar
    pPlayer->SetSelectedAvatarBits(0);
    pPlayer->SetSelectedAvatarKey(INVALID_INDEX);
    ...
    CBufferFixed<7> FullBuffer;
    CBufferFixed<7> EnemyBuffer;
    FullBuffer << GAME_CMD_RANDOM_HERO_MESSAGE << iClientNumber << unHero;
...
void    CGameServer::SelectPotentialHeroOrRandomHero(int iClientNumber)
{
    ...
    if (!pPlayer->HasSelectedHero() && pPlayer->HasPotentialHero())
        SelectHero(iClientNumber, pPlayer->GetPotentialHero(), false, false);
...
/*====================
  ...
  ====================*/
void    CGameServer::Precache(const tstring &sName, EPrecacheScheme eScheme, const tstring &sModifier)
{
    EntityRegistry.ServerPrecache(EntityRegistry.LookupID(sName), eScheme, sModifier);
}
...
/*====================
  CGameServer::Precache
  ====================*/
void    CGameServer::Precache(ushort unType, EPrecacheScheme eScheme, const tstring &sModifier)
{
    EntityRegistry.ServerPrecache(unType, eScheme, sModifier);
}
...
void    CGameServer::SendUnitPing(const CPing *pPing, IUnitEntity *pSource, IUnitEntity *pTarget)
{
    ...
    for (PlayerMap_it itPlayer(m_mapClients.begin()); itPlayer != m_mapClients.end(); ++itPlayer)
    {
        ...
        if (pPlayer->IsDisconnected())
            continue;
        if (pPlayer->GetTeam() != TEAM_SPECTATOR && pPlayer->GetTeam() != uiSourceTeam)
            continue;
...
void    CGameServer::SendUnitPing(byte yType, IUnitEntity *pSource, IUnitEntity *pTarget)
{
    SendUnitPing(Game.GetPing(yType), pSource, pTarget);
}

void    CGameServer::SendUnitPing(EPing eType, IUnitEntity *pSource, IUnitEntity *pTarget)
{
    SendUnitPing(Game.GetPing(eType), pSource, pTarget);
}
...
{
    m_mapPlayerAccountStats[iAccountID] = CPlayerAccountStats(pData);
}
#endif

#ifndef K2_CLIENT
/*====================
  CGameServer::ProcessAuxData
  ====================*/
void    CGameServer::ProcessAuxData(int iClientNum, const CPHPData *pData)
{
    CPlayer *pPlayer(GetPlayer(iClientNum));
    ...
    pPlayer->SetAccountHistory(&accHistory);
}
#endif
...
/*====================
  CGameServer::UpdateUpgrades
  ====================*/
void    CGameServer::UpdateUpgrades(int iClientNum)
{
    CPlayer *pPlayer(GetPlayer(iClientNum));
    CClientConnection *pClientConnection(m_pHostServer->GetClient(iClientNum));
