...
#include "../aba_shared/i_areaaffector.h"
#include "../aba_shared/c_linearaffector.h"
#include "../aba_shared/i_projectile.h"
...
CMD(MinimapPing)
{
	...
	buffer << GAME_CMD_MINIMAP_PING
		<< byte(AtoF(vArgList[0]) * UCHAR_MAX)
		<< byte(AtoF(vArgList[1]) * UCHAR_MAX);
	GameClient.SendGameData(buffer, false);
	return true;
}
...
CMD(CallVote)
{
	...
	if (eVote == VOTE_TYPE_INVALID || eVote == VOTE_TYPE_KICK)
		return false;

	int iParam(-1);
	if (eVote == VOTE_TYPE_KICK_AFK)
	{
		if (vArgList.size() > 1)
		{
...
UI_CMD(MinimapClick, 2)
{
	...
	return XtoA(pCommander->MinimapPrimaryClick(CVec2f(fX, fY)));
}
...
UI_CMD(MinimapRightClick, 2)
{
	...
	return XtoA(pCommander->MinimapSecondaryClick(CVec2f(fX, fY)));
}
...
{
	if (vArgList.size() > 0)
		GameClient.PrecacheWorld(vArgList[0]);
	return true;
}
...
UI_VOID_CMD(AddPlayers, 1)
{
	...
	for (uint uiTeam(0); uiTeam < 2; ++uiTeam)
	{
		...
		for (uint uiPlayer(0); uiPlayer < 5; ++uiPlayer)
		{
...
{
	return XtoA(GameClient.GetCurrentGamePointer()->GetGamePhase());
}
...
UI_CMD(IsInGame, 0)
{
	...
	if (GameClient.GetCurrentGamePointer()->GetGamePhase() > 1 && GameClient.GetCurrentGamePointer()->GetGamePhase() < 7)
		return _T("1");
	else
		return _T("0");
}
