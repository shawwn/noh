...
#include "../k2/i_resourcewatcher2.h"
//=============================================================================
...
struct SWeatherInfo
{
	...
	tstring		sLocalizedName;		// example: "Acid Rain" or "Chuva Ácida" or ... etc
...
enum EClientResource
{
	...
	CLIENT_RESOURCE_PING_PROTECT,
	CLIENT_RESOURCE_PING_ATTACK,
	CLIENT_RESOURCE_PING_ATTACK_HERO,

	CLIENT_RESOURCE_EXP_INDICATOR_EFFECT,
...
struct SWorldThingSpawnRequest
{
	EWorldThing	m_eType;
	uint		m_uiWorldIndex;
	uint		m_uiGameIndex;
	...
	SWorldThingSpawnRequest(EWorldThing eType, uint uiWorldIndex, uint uiGameIndex, uint uiBitIndex = 0) :
	m_eType(eType),
...
struct SVoiceSet
{
	...
	ResHandle 	hDenied;
	ResHandle 	hGetItOn;
	ResHandle 	hHellbourneWins;
	ResHandle 	hLegionWins;
	ResHandle 	hVictory;
	ResHandle 	hDefeat;
	ResHandle 	hRival;
	ResHandle 	hPayback;
	ResHandle 	hSmackdown;
	ResHandle 	hHumiliation;
	ResHandle 	hImmortal;
	ResHandle 	hRageQuit;
	ResHandle 	hHellbourneDestroyedTower;
	ResHandle 	hLegionDestroyedTower;
	ResHandle 	hHellbourneBarracksDestroyed;
	ResHandle 	hLegionBarracksDestroyed;
	ResHandle 	hBaseUnderAttack;
	ResHandle 	hAllyHeroUnderAttack;
	ResHandle 	hOurHeroUnderAttack;
	ResHandle 	hUnpausing;
	ResHandle 	hKongorSlain;

	SVoiceSet() :
	hStartGame(INVALID_INDEX),
	hFirstBlood(INVALID_INDEX),
	hTeamKillstreak(INVALID_INDEX),
	hTeamWipe(INVALID_INDEX),
	hDenied(INVALID_INDEX),
	hGetItOn(INVALID_INDEX),
	hHellbourneWins(INVALID_INDEX),
	hLegionWins(INVALID_INDEX),
	hVictory(INVALID_INDEX),
	hDefeat(INVALID_INDEX),
	hRival(INVALID_INDEX),
	hPayback(INVALID_INDEX),
	hSmackdown(INVALID_INDEX),
	hHumiliation(INVALID_INDEX),
	hImmortal(INVALID_INDEX),
	hRageQuit(INVALID_INDEX),
	hHellbourneDestroyedTower(INVALID_INDEX),
	hLegionDestroyedTower(INVALID_INDEX),
	hHellbourneBarracksDestroyed(INVALID_INDEX),
	hLegionBarracksDestroyed(INVALID_INDEX),
	hBaseUnderAttack(INVALID_INDEX),
	hAllyHeroUnderAttack(INVALID_INDEX),
	hOurHeroUnderAttack(INVALID_INDEX),
...
typedef deque<SWorldThingSpawnRequest>	WorldThingList;
...
private:
	...
	HeroPrecacheList		m_deqReplayHeroResources;
	bool					m_bStartedLoadingResources;
	bool					m_bStartedLoadingEntityResources;
	uint					m_uiTotalResourcesToLoad;
	ResHandle				m_ahResources[NUM_CLIENT_RESOURCES];
	vector<SVoiceSet>		m_vAnnouncerVoices;
	...
	bool					m_bNeedLoadTerrainTypes;
	uint					m_uiLastUnitIndex;
	...
	// Weather
	WeatherInfoMap			m_mapActiveWeatherEffects;
	uint					m_uiWeatherEffectCounter;
	...
	tstring		GetVoicePath(const tstring &sSet, const tstring &sKey);
	void		AddAnnouncerVoiceSampleToLoadingQueue(const tstring &sSet, const tstring &sKey);
...
public:
	...
	void				StartPreloadingWorld();
	void				SpawnNextWorldEntity();
	void				PrecacheNextWorldEntity();
	...
	byte				GetHeroStatus(ushort unHeroTypeID) const;
	...
	int					StartClientGameEffect(ResHandle hEffect, int iChannel, int iTimeNudge, const CVec3f &v3Pos);
	...
	void				StartLoading(const tstring &sWorldName);
	void				FinishLoading();
	...
	uint				CalcAvailableWeatherEffects(vector<SWeatherInfo>& vWeather);
	bool				IsWeatherEffectName(const tstring& sStartWeather);
	uint				StartWeatherByName(const tstring& sStartWeather);
	uint				StartWeather(const SWeatherInfo& cWeatherInfo);
	uint				StopWeatherByName(const tstring& sName);
	void				StopWeather(uint uiIdx);
	void				StopAllWeather();
};
//=============================================================================
