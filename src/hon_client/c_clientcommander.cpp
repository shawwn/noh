...
#include "../hon_shared/i_propentity.h"
...
m_hPingAlly(INVALID_RESOURCE),
m_hPingEnemy(INVALID_RESOURCE),
...
m_hCurrentCursor(INVALID_RESOURCE),
...
void    CClientCommander::LoadResources(ResHandle hClientSoundsStringTable)
{
    ...
    m_hPingAlly = g_ResourceManager.LookUpName(_T("ping_ally"), RES_K2CURSOR);
    m_hPingEnemy = g_ResourceManager.LookUpName(_T("ping_enemy"), RES_K2CURSOR);
...
void    CClientCommander::Frame()
{
    ...
    ResHandle hCurrentCursor(INVALID_RESOURCE);
    ...
    if (pInterface)
    {
        pWidget = pInterface->GetHoverWidget();
        if(pWidget)
        {
    ...
    else if (GetCommanderState() == COMSTATE_ATTACK)
    {
        ...
        else
        {
            if (pHover->IsTargetType(TARGET_TRAIT_ENEMY, pUnit))
                ...
                hCurrentCursor = m_hActionAllyInvalid;
        }
    }
    ...
    else if (bIsUnitPingModifierDown  && WType != WIDGET_MAP && pHover && pLocalPlayer && pLocalPlayer->GetTeam() != TEAM_SPECTATOR && pLocalPlayer->IsEnemy(pHover) && ((pHover->IsBuilding() && !pHover->GetAsBuilding()->GetIsShop() && !pHover->GetAsBuilding()->GetNoAltClickPing()) || pHover->IsHero()))
    {
        hCurrentCursor = m_hPingEnemy;
    }
    else if (bIsUnitPingModifierDown && WType != WIDGET_MAP && pHover && pLocalPlayer && pLocalPlayer->GetTeam() != TEAM_SPECTATOR && !pLocalPlayer->IsEnemy(pHover) && pHover->IsBuilding() && !pHover->GetAsBuilding()->GetIsShop() && !pHover->GetAsBuilding()->GetNoAltClickPing())
    {
        hCurrentCursor = m_hPingAlly;
    }
    else if (GameClient.GetMinimapHoverUnit() != -1)
    {
        uint uiMiniHoverUnit(GameClient.GetMinimapHoverUnit());
        IUnitEntity* pUnit(GameClient.GetUnitEntity(uiMiniHoverUnit));
        
        if (bIsUnitPingModifierDown && pUnit && pLocalPlayer && pLocalPlayer->GetTeam() != TEAM_SPECTATOR && pLocalPlayer->IsEnemy(pUnit) && ((pUnit->IsBuilding() && !pUnit->GetAsBuilding()->GetIsShop() && !pUnit->GetAsBuilding()->GetNoAltClickPing()) || pUnit->IsHero()))
        {
            hCurrentCursor = m_hPingEnemy;
        }
        else if (bIsUnitPingModifierDown &&  pUnit && pLocalPlayer && pLocalPlayer->GetTeam() != TEAM_SPECTATOR && !pLocalPlayer->IsEnemy(pUnit) && pUnit->IsBuilding() && !pUnit->GetAsBuilding()->GetIsShop() && !pUnit->GetAsBuilding()->GetNoAltClickPing())
        {
            hCurrentCursor = m_hPingAlly;
        }
    }
    ...
    if (m_hCurrentCursor != hCurrentCursor)
    {
        Input.SetCursor(CURSOR_GAME, hCurrentCursor);
        m_hCurrentCursor = hCurrentCursor;
    }
}
...
void    CClientCommander::PrimaryUp(const CVec2f &v2Cursor)
{
    ...
    case COMSTATE_TARGET_VECTOR:
        if (m_bVectorTargeting &&
            DistanceSq(m_v3VectorTargetPos.xy(), m_v3TraceEndPos.xy()) > SQR<float>(cg_cmdrVectorTargetMinDistance))
        {
...
void    CClientCommander::GiveOrder(ECommanderOrder eOrder, uint uiTargetIndex, byte yQueue, uint uiParam)
{
    ...
    buffer
        ...
        << byte(GetModifier4() ? CMDR_ORDER_FLAG_DIRECT_PATHING : 0);
    GameClient.SendGameData(buffer, true);
}
...
void    CClientCommander::Ping()
{
    ...
    if (WType == WIDGET_MAP && GameClient.GetMinimapHoverUnit() != INVALID_INDEX && GetPingEnabled() && pLocalPlayer->GetTeam() != TEAM_SPECTATOR)
    {
        pHover = GameClient.GetUnitEntity(GameClient.GetMinimapHoverUnit());

        if(pHover && ((pHover->IsBuilding() && !pHover->GetAsBuilding()->GetNoAltClickPing() && (pHover->GetTeam() == pLocalPlayer->GetTeam() || pLocalPlayer->IsEnemy(pHover)) && !pHover->GetAsBuilding()->GetIsShop() && !pHover->GetAsBuilding()->GetNoAltClickPing()) || 
        (pHover->IsHero() && pLocalPlayer->IsEnemy(pHover))))
        {
            CBufferFixed<5> buffer;
            buffer << GAME_CMD_UNIT_PING
                << GameClient.GetMinimapHoverUnit();
            GameClient.SendGameData(buffer, false);
        }
    }
    else if (pWidget && WType == WIDGET_MAP && GetPingEnabled())
    {
        CMinimap* pMinimap(static_cast<CMinimap*>(pWidget));
        CVec2f vPlusPos(0.0f, 0.0f);
        IWidget* pParent(pMinimap->GetParent());
        while (pParent)
        {
            vPlusPos += pParent->GetPos();
            pParent = pParent->GetParent();
        }
        CVec2f vPos(pMinimap->GetCursorInWorld(Input.GetCursorPos() - vPlusPos));

        CBufferFixed<3> buffer;
        buffer << GAME_CMD_MINIMAP_PING
            << byte(vPos.x * UCHAR_MAX)
            << byte(vPos.y * UCHAR_MAX);
        GameClient.SendGameData(buffer, false);
    }
    else if (pHover && pLocalPlayer->GetTeam() != TEAM_SPECTATOR && GetPingEnabled() &&
