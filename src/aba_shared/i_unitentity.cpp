...
DEFINE_ENTITY_DESC(IUnitEntity, 1)
{
    ...
    s_cDesc.pFieldTypes->push_back(SDataField(_T("m_unUnitFlags"), TYPE_SHORT, 11, 0));
    ...
    s_cDesc.pFieldTypes->push_back(SDataField(_T("m_fStamina"), TYPE_FLOOR16, 15, 0));
...
void    IUnitEntity::GetSnapshot(CEntitySnapshot &snapshot, uint uiFlags) const
{
    ...
    else
    {
        ...
        snapshot.WriteFloor16(m_fStamina);
    }
...
bool    IUnitEntity::ReadSnapshot(CEntitySnapshot &snapshot, uint uiVersion)
{
    try
    {
        ...
        snapshot.ReadFloat16(m_fStamina);
...
/*====================
  IUnitEntity::Touch
  ====================*/
void    IUnitEntity::Touch(IGameEntity *pActivator)
{
    if (pActivator == NULL)
        return;

    if (pActivator->IsUnit() && pActivator->GetAsUnit()->GetOwnerClientNumber() != -1)
    {
        CBufferFixed<5> buffer;
        buffer << GAME_CMD_TOUCH << ushort(GetIndex()) << ushort(pActivator->GetIndex());
        Game.SendGameData(pActivator->GetAsUnit()->GetOwnerClientNumber(), buffer, true);
    }
...
void    IUnitEntity::Copy(const IGameEntity &B)
{
    ...
    m_fCurrentMaxStamina = C.m_fCurrentMaxStamina;
}
...
void    IUnitEntity::SnapshotUpdate()
{
    ...
    m_fCurrentMaxStamina = GetMaxStamina();
...
/*====================
  ...
  ====================*/
void    IUnitEntity::ClientPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme)
{
    IVisualEntity::ClientPrecache(pConfig, eScheme);
...
/*====================
  ...
  ====================*/
void    IUnitEntity::ServerPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme)
{
    IVisualEntity::ServerPrecache(pConfig, eScheme);
...
bool    IUnitEntity::ServerFrameSetup()
{
    ...
    bool bAttemptSprint(false);
    float fSprintStaminaCost(0.0f);
    ...
    if (GetStatus() == ENTITY_STATUS_ACTIVE)
    {
        ...
        if (pPlayer != NULL && pPlayer->GetSprinting())
            bAttemptSprint = true;
    ...
    if (bAttemptSprint)
    {
        fSprintStaminaCost = GetSprintStaminaCost() * fFrameTime;

        if (m_fStamina >= fSprintStaminaCost)
            SetUnitFlags(UNIT_FLAG_SPRINTING);
        else
            RemoveUnitFlags(UNIT_FLAG_SPRINTING);

        // Drain stamina even if we don't have enough stamina to sprint so the
        // sprint state doesn't bounce
        TakeStamina(fSprintStaminaCost);
    }
    else
    {
        RemoveUnitFlags(UNIT_FLAG_SPRINTING);
...
bool    IUnitEntity::ServerFrameCleanup()
{
    ...
    float fNewMaxStamina(GetMaxStamina());
    if (m_fCurrentMaxStamina != fNewMaxStamina)
    {
        if (m_fCurrentMaxStamina > 0.0f)
            m_fStamina = (m_fStamina / m_fCurrentMaxStamina) * fNewMaxStamina;
        else
            m_fStamina = 0.0f;

        m_fCurrentMaxStamina = fNewMaxStamina;
    }
...
int     IUnitEntity::TransferItem(int iClientNum, IEntityItem *pItem, int iSlot)
{
    ...
    if (iNewSlot != -1)
    {
        if (iNewSlot == INVENTORY_BACKPACK_PROVISIONAL)
...
bool    IUnitEntity::CanCarryItem(IEntityItem *pItem)
{
    ...
    // TODO: look into this.  I'm commenting this out temporarily, since it was causing 
    // engineer's turret to drop its items (dupe item bug).
    //if (!GetCanCarryItems())
        //return false;
...
void    IUnitEntity::Spawn()
{
    ...
    m_fStamina = m_fCurrentMaxStamina = GetMaxStamina();
...
void    IUnitEntity::KillReward(IUnitEntity *pKiller, CPlayer *pPlayerKiller)
{
    ...
    else
    {
        ...
        vEntities.clear();
        Game.GetEntitiesInRadius(vEntities, GetPosition().xy(), g_experienceRange, REGION_UNIT);
...
bool    IUnitEntity::Attack(IUnitEntity* pTarget, bool bAttackAbility)
{
    ...
    if (m_unAttackProjectile == INVALID_ENT_TYPE && m_hAttackImpactEffect != INVALID_RESOURCE)
    {
        CGameEvent ev;
                                           ...
                                           bool bDeathAnim, bool bInheritActions)
{
    ...
    pIllusion->m_fCurrentMaxStamina = m_fCurrentMaxStamina;

    pIllusion->SetHealth(GetHealth());
    pIllusion->SetMana(GetMana());
    pIllusion->SetStamina(GetStamina());
...
void    IUnitEntity::Interpolate(float fLerp, IVisualEntity *pPrevState, IVisualEntity *pNextState)
{
    ...
    // Stamina
    if (pPrev->m_fCurrentMaxStamina > 0.0f && pNext->m_fCurrentMaxStamina > 0.0f)
    {
        float fPrevStaminaFrac(pPrev->m_fStamina / pPrev->m_fCurrentMaxStamina);
        float fNextStaminaFrac(pNext->m_fStamina / pNext->m_fCurrentMaxStamina);

        m_fStamina = LERP(fLerp, fPrevStaminaFrac, fNextStaminaFrac) * m_fCurrentMaxStamina;
    }
    else
    {
        m_fStamina = pNext->m_fStamina;
    }
...
/*====================
  IUnitEntity::GiveStamina
  ====================*/
void    IUnitEntity::GiveStamina(float fValue)
{
    m_fStamina = CLAMP(m_fStamina + fValue, 0.0f, GetMaxStamina());
}


/*====================
  IUnitEntity::SpendStamina
  ====================*/
bool    IUnitEntity::SpendStamina(float fCost)
{
    if (fCost > m_fStamina)
        return false;
    else
...
/*====================
  IUnitEntity::TakeStamina
  ====================*/
void    IUnitEntity::TakeStamina(float fValue)
{
    m_fStamina = CLAMP(m_fStamina - fValue, 0.0f, GetMaxStamina());
}
...
/*====================
  IUnitEntity::RegenerateStamina
  ====================*/
void    IUnitEntity::RegenerateStamina(float fFrameTime)
{
    m_fStamina = CLAMP(m_fStamina + fFrameTime * GetStaminaRegen(), 0.0f, m_fCurrentMaxStamina);
}
