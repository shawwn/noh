...
int		IEntityItem::GetValue() const
{
	...
	if (GetInitialCharges() == 0)
		return GetTotalCost() * g_itemSellValue;

	// Selling an item with charges
	return (GetTotalCost() * g_itemSellValue) * (GetCharges() / float(GetInitialCharges()));
}
...
/*====================
  ...
  ====================*/
void	IEntityItem::ClientPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme)
{
	IEntityTool::ClientPrecache(pConfig, eScheme);
...
/*====================
  ...
  ====================*/
void	IEntityItem::ServerPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme)
{
	IEntityTool::ServerPrecache(pConfig, eScheme);
}
