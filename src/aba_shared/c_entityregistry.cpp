...
/*====================
  ...
  ====================*/
void    CEntityRegistry::ServerPrecache(ushort unType, EPrecacheScheme eScheme) const
{
    try
    {
        ...
        {
            citStatic->second->ServerPrecache(eScheme);
            return;
        }
        ...
        if (citDynamic != m_mapDynamicTypeIDs.end())
        {
            ...
            if (pDefRes == NULL || pDefRes->GetDefinition<IEntityDefinition>() == NULL)
                return;
            pDefRes->GetDefinition<IEntityDefinition>()->Precache(eScheme);
            citDynamic->second.ServerPrecache(eScheme);
        }
...
/*====================
  ...
  ====================*/
void    CEntityRegistry::ClientPrecache(ushort unType, EPrecacheScheme eScheme) const
{
    EntAllocatorIDMap_cit   citFind(m_mapAllocatorIDs.find(unType));
    if (citFind != m_mapAllocatorIDs.end())
    {
        citFind->second->ClientPrecache(eScheme);
        return;
    }
    ...
    if (citDynamic != m_mapDynamicTypeIDs.end())
    {
        ...
        pDefRes->GetDefinition<IEntityDefinition>()->Precache(eScheme);
        citDynamic->second.ClientPrecache(eScheme);
        return;
    }
...
CScriptThread*  CEntityRegistry::NewScriptThread(const tstring &sName)
{
    ...
    CScriptThread *pNewThread(K2_NEW(global,    CScriptThread)(sName));
...
{
    for (map<tstring, CScriptThread *>::iterator it(m_mapDefinitions.begin()), itEnd(m_mapDefinitions.end()); it != itEnd; ++it)
        it->second->Precache(PRECACHE_ALL);
}
