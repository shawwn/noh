...
void	CBrain::Init()
{
	...
	m_pActionStates[ASID_ATTACKING] = K2_NEW(g_heapAI,    CASAttacking)(*this);
	m_pActionStates[ASID_CASTING] = K2_NEW(g_heapAI,    CASCasting)(*this);
	m_pActionStates[ASID_MOVING] = K2_NEW(g_heapAI,    CASMoving)(*this);
...
bool	CBrain::ProcessCommand(const SUnitCommand &cmd)
{
	...
	{
	case UNITCMD_STOP:
		pBehavior = K2_NEW(g_heapAI,    CBStop)();
		break;

	case UNITCMD_HOLD:
		pBehavior = K2_NEW(g_heapAI,    CBHold)(cmd.uiParam);
		break;

	case UNITCMD_MOVE:
		pBehavior = K2_NEW(g_heapAI,    CBMove)();
		break;

	case UNITCMD_ATTACK:
		pBehavior = K2_NEW(g_heapAI,    CBAttack)(m_pUnit->GetArmingSequence());
		break;

	case UNITCMD_ATTACKMOVE:
		pBehavior = K2_NEW(g_heapAI,    CBAttackMove)();
		break;

	case UNITCMD_ABILITY:
		pBehavior = K2_NEW(g_heapAI,    CBAbility)(cmd.uiParam, false);
		break;

	case UNITCMD_ABILITY2:
		pBehavior = K2_NEW(g_heapAI,    CBAbility)(cmd.uiParam, true);
		break;

	case UNITCMD_DOUBLE_ACTIVATE_ABILITY:
		pBehavior = K2_NEW(g_heapAI,    CBDoubleActivateAbility)(cmd.uiParam);
		break;

	case UNITCMD_FOLLOW:
		pBehavior = K2_NEW(g_heapAI,    CBFollow)();
		break;

	case UNITCMD_DROPITEM:
		pBehavior = K2_NEW(g_heapAI,    CBDropItem)(cmd.uiParam);
		break;

	case UNITCMD_GIVEITEM:
		pBehavior = K2_NEW(g_heapAI,    CBGiveItem)(cmd.uiParam);
		break;

	case UNITCMD_TOUCH:
		pBehavior = K2_NEW(g_heapAI,    CBTouch)();
		break;

	case UNITCMD_GUARD:
		pBehavior = K2_NEW(g_heapAI,    CBGuard)();
		break;

	case UNITCMD_WANDER:
		pBehavior = K2_NEW(g_heapAI,    CBWander)();
		break;

	case UNITCMD_AGGRESSIVEWANDER:
		pBehavior = K2_NEW(g_heapAI,    CBAggressiveWander)();
		break;

	case UNITCMD_AGGRO:
		pBehavior = K2_NEW(g_heapAI,    CBAggro)();
		break;

	case UNITCMD_EVENT:
		pBehavior = K2_NEW(g_heapAI,    CBEvent)();
		break;

	case UNITCMD_GUARDFOLLOW:
		pBehavior = K2_NEW(g_heapAI,    CBGuardFollow)();
		break;

	case UNITCMD_FOLLOWGUARD:
		pBehavior = K2_NEW(g_heapAI,    CBFollowGuard)();
		break;

	case UNITCMD_SENTRY:
		pBehavior = K2_NEW(g_heapAI,    CBSentry)();
		break;

	case UNITCMD_ASSIST:
		pBehavior = K2_NEW(g_heapAI,    CBAssist)();
		break;

	case UNITCMD_ATTACKFOLLOW:
		pBehavior = K2_NEW(g_heapAI,    CBAttackFollow)();
		break;
