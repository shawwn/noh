...
public:
	...
	virtual ResHandle	RegisterGameMechanics(const tstring &sPath)		{ return RegisterGameMechanics(g_ResourceManager.Register(sPath, RES_GAME_MECHANICS)); }
	bool				FetchGameMechanics()							{ CGameMechanicsResource *pResource(g_ResourceManager.Get<CGameMechanicsResource>(m_hGameMechanics)); m_pGameMechanics = pResource ? pResource->GetMechanics() : NULL; return m_pGameMechanics != NULL; }
	...
	virtual ResHandle		RegisterSample(const tstring &sPath)	{ return g_ResourceManager.LookUpPath(sPath); }
	virtual ResHandle		RegisterMaterial(const tstring &sPath)	{ return g_ResourceManager.LookUpPath(sPath); }

	GAME_SHARED_API void	Precache(ResHandle hDefinition, EPrecacheScheme eScheme);
	virtual void			Precache(const tstring &sName, EPrecacheScheme eScheme) = 0;
	virtual void			Precache(ushort unType, EPrecacheScheme eScheme) = 0;
	virtual void			GetPrecacheList(const tstring &sName, EPrecacheScheme eScheme, HeroPrecacheList &deqPrecache)	{}
	...
	virtual void	LogAward(EGameLogEvent eEvent, IUnitEntity *pAttacker, IUnitEntity *pTarget)										{}
