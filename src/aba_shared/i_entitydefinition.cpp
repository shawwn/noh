...
{
    m_sName = sName;
    for (map<ushort, IEntityDefinition*>::iterator it(m_mapModifiers.begin()); it != m_mapModifiers.end(); ++it)
...
{
    CCombatActionScript *pNewScript(K2_NEW(g_heapResources,   CCombatActionScript)(eScript, iPriority, bPropagateToIllusions, bActivateOnBounces, m_uiModifierID));
    if (pNewScript == NULL)
        return NULL;
...
/*====================
  ...
  ====================*/
void    IEntityDefinition::Precache(EPrecacheScheme eScheme)
{
    PRECACHE_GUARD
...
/*====================
  ...
  ====================*/
void    IEntityDefinition::GetPrecacheList(EPrecacheScheme eScheme, HeroPrecacheList &deqPrecache)
{
    PRECACHE_GUARD
        // Script actions (cast projectiles, affectors, summoned units, etc)
        for (uint uiScript(0); uiScript < NUM_ACTION_SCRIPTS; ++uiScript)
        {
            ...
            pScript->GetPrecacheList(eScheme, deqPrecache);
        }
        ...
        for (map<ushort, IEntityDefinition*>::iterator it(m_mapModifiers.begin()); it != m_mapModifiers.end(); ++it)
            it->second->GetPrecacheList(eScheme, deqPrecache);
    PRECACHE_GUARD_END
}
...
IEntityDefinition*  IEntityDefinition::GenerateMergedModifier(ushort unModifierBits)
{
    ...
    for (ushort un(1); un != 0; un <<= 1)
    {
        ...
        map<ushort, IEntityDefinition*>::iterator itModifier(m_mapModifiers.find(un));
...
IEntityDefinition*  IEntityDefinition::GetModifiedDefinition(ushort unModifierBits)
{
    ...
    map<ushort, IEntityDefinition*>::iterator itFind(m_mapModifiers.find(unModifierBits));
