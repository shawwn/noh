...
static void UpdateNetworkResource(const string &sStateUTF8, const string &sValueUTF8)
{
    ...
    NetworkResourceManager.RegisterNetworkResource(g_ResourceManager.Register(sPath, eType), uiIndex);
}
...
bool    CReplayManager::StartPlayback(const tstring &sFilename, bool bGenerateKeyFrames)
{
    try
    {
        ...
        IModalDialog::SetNumLoadingJobs(8);
        ...
        // Open archive
        m_cAchive.Open(sFilename);

        if (!m_cAchive.IsOpen())
            EX_WARN(_T("Failed to open archive"));
        
        // Open replay data
        m_hReplayData.Open(_T("ReplayData"), FILE_READ | FILE_BINARY | FILE_UTF16, m_cAchive);
