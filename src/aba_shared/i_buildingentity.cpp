...
CVAR_FLOATF (g_buildingMapIconSize,             0.035f, CVAR_TRANSMIT | CVAR_GAMECONFIG);
...
{
    m_pSkeleton = K2_NEW(g_heapSkeleton,   CSkeleton);
    return m_pSkeleton;
}
...
{
    Game.Precache(m_unType, PRECACHE_ALL); // TODO: Make this smarter...
    ...
    if (GetAttackType() != INVALID_ATTACK_TYPE)
        m_cBrain.AddBehavior(K2_NEW(g_heapAI,   CBSentry)(), false);
...
/*====================
  ...
  ====================*/
void    IBuildingEntity::ClientPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme)
{
...
/*====================
  ...
  ====================*/
void    IBuildingEntity::ServerPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme)
{
...
{
    if (g_towerHeroArmorReduction && GetIsTower())
    {
        CTeamInfo *pTeam(Game.GetTeam(GetTeam()));
        if (pTeam == NULL)
