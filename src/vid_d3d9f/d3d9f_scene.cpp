...
void    D3D_AddEffectQuadBatches(EMaterialPhase ePhase)
{
    ...
    if (g_dwEffectQuadBase + iNumEffectQuads >= MAX_EFFECT_QUADS)
    ...
    for (int n(0); n < g_iNumBillboards; ++n)
    {
        ...
        g_RenderList.Add(new CEffectQuadRenderer(hMaterial, g_dwEffectQuadBase + n, g_dwEffectQuadBase + f, (iMaxEffectLayer << 20) + n, -FAR_AWAY));
    ...
    for (int n = 0; n < g_iNumBeams; ++n)
    {
        ...
        g_RenderList.Add(new CEffectQuadRenderer(hMaterial, g_dwEffectQuadBase + iBufferOffset + n, g_dwEffectQuadBase + iBufferOffset + f, (iMaxEffectLayer << 20) + n, -FAR_AWAY));
...
void    D3D_AddBoxBatches()
{
    ...
    g_RenderList.Add(new CBoxRenderer(g_iNumBoxes));
}
...
void    D3D_AddPointBatches()
{
    ...
    g_RenderList.Add(new CPointRenderer(g_iNumPoints));
}
...
void    D3D_AddLineBatches()
{
    ...
    g_RenderList.Add(new CLineRenderer(g_iNumLines));
}
...
void    D3D_AddEffectTriangleBatches(EMaterialPhase ePhase)
{
    ...
    if (g_dwEffectTriangleBase + g_iNumEffectTriangles >= MAX_EFFECT_TRIANGLES)
    ...
    for (int n(0); n < g_iNumEffectTriangles; ++n)
    {
        ...
        g_RenderList.Add(new CEffectTriangleRenderer(hMaterial, g_dwEffectTriangleBase + n, g_dwEffectTriangleBase + f, (iMinEffectLayer << 20) + n, -FAR_AWAY));
...
void    D3D_AddExtendedTriangleBatches(EMaterialPhase ePhase)
{
    ...
    for (int n(0); n < g_iNumExtendedTriangles; ++n)
    {
        ...
        g_RenderList.Add(new CExtendedTriangleRenderer(hMaterial, n, f, 0, -FAR_AWAY));
...
void    D3D_AddTreeBillboardBatches(EMaterialPhase ePhase)
{
    ...
    for (int n(0); n < g_iNumTreeBillboards; ++n)
    {
        ...
        g_RenderList.Add(new CTreeBillboardRenderer(hMaterial, dwAlphaTest, n, f, 0, 0.0f));
...
void    D3D_AddScenePolys(EMaterialPhase ePhase)
{
    ...
    {
        g_RenderList.Add(new CScenePolyRenderer(it->hMaterial, it->verts, it->zNumVerts, it->flags));
    }
}
...
void    D3D_AddSky()
{
    ...
    if (SceneManager.GetDrawSkybox())
        g_RenderList.Add(new CSkyRenderer());
}
...
void    D3D_DrawQuads()
{
    ...
    if (g_dwGuiQuadBase + g_iNumGuiQuads >= MAX_GUIQUADS)
        g_dwGuiQuadBase = 0;
