...
// disable 'fopen' compiler warning.
...
#else
#define ToLittle(x) x = SwapEndian(x);
inline unsigned int SwapEndian(unsigned int in)
{
	return ((in >> 24) & 255) +
		   (((in >> 16) & 255) << 8) +
		   (((in >> 8) & 255) << 16) +
...
unsigned int
	SOIL_load_OGL_single_texture3D_from_memory
	(
		const unsigned char *const buffer,
		int buffer_length,
		int depth,
	...
	)
{
	...
	if( buffer == NULL )
	{
		result_string_pointer = "Invalid single texture3D buffer";
		return 0;
	}
	/*	does the user want direct uploading of the image as a DDS file?	*/
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	...
	/* only supports direct loading for now */
	return 0;
}
		...
		int loading_as_cubemap )
{
	...
	ToLittle(header.dwMagic);
	ToLittle(header.dwSize);
	ToLittle(header.dwFlags);
	ToLittle(header.dwHeight);
	ToLittle(header.dwWidth);
	ToLittle(header.dwMipMapCount);
	ToLittle(header.dwDepth);
	ToLittle(header.sPixelFormat.dwSize);
	ToLittle(header.sPixelFormat.dwFlags);
	ToLittle(header.sPixelFormat.dwFourCC);
	ToLittle(header.sCaps.dwCaps1);
	ToLittle(header.sCaps.dwCaps2);
	//ToLittle(header.)
	...
	volume = (header.sCaps.dwCaps2 & DDSCAPS2_VOLUME) / DDSCAPS2_VOLUME;
	...
	if( uncompressed )
	{
		...
		DDS_main_size = width * height * depth * block_size;
	} else
	{
		if( flags & SOIL_FLAG_DDS_DECOMPRESS )
		{
			result_string_pointer = "Direct upload of compressed S3TC image skipped";
			return 0;
		}
		...
		case 5:
			...
			block_size = 16;
			if( (flags & SOIL_FLAG_DDS_DECOMPRESS_DXT5) )
			{
				result_string_pointer = "Direct upload of compressed DXT5 image skipped";
				return 0;
			}
			break;
		}
	...
	{
		if( loading_as_cubemap != 2 )
		{
			/*	we can't do it!	*/
			result_string_pointer = "DDS image was a volume texture";
			return 0;
		}
		ogl_target_start = GL_TEXTURE_3D;
		ogl_target_end = GL_TEXTURE_3D;
		opengl_texture_type = GL_TEXTURE_3D;
	} else
	{
		...
		{
			/*	we can't do it!	*/
			result_string_pointer = "DDS image was a texture";
			return 0;
		}
	...
	if( (header.sCaps.dwCaps1 & DDSCAPS_MIPMAP) && (header.dwMipMapCount > 1) )
	{
		...
		for( i = 1; i <= mipmaps; ++ i )
		{
			...
			DDS_full_size += w*h*d*block_size;
		}
	} else
	{
		mipmaps = 0;
