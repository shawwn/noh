...
/*====================
  ...
  ====================*/
void    CTextureArchiveNode::WriteTexture(const tstring &sPath, uint uiTextureID, CArchive &cArchive, bool bOverwrite)
{
#if 0
...
{
    m_vTextureArchiveNodes.push_back(K2_NEW(ctx_GL2,    CTextureArchiveNode)(sDirectory, sDefines));
...
/*====================
  ...
  ====================*/
void    CTextureArchive::WriteTexture(const tstring &sPath, uint uiTextureID, bool bOverwrite)
{
    if (!m_bInitialized)
        return;
    ...
    if (!sPath.empty() && sPath[0] == _T('$'))
        m_vTextureArchiveNodes[m_uiActiveNode]->WriteTexture(_T("/") + sPath.substr(1), uiTextureID, m_cArchiveWrite, bOverwrite);
    else
        m_vTextureArchiveNodes[m_uiActiveNode]->WriteTexture(sPath, uiTextureID, m_cArchiveWrite, bOverwrite);
}
