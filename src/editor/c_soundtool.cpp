...
void    CSoundTool::UpdateSounds()
{
    for(SoundsMap_it it = m_mapSounds.begin(); it != m_mapSounds.end(); it++)
    {
        ...
        ResHandle hSound(g_ResourceManager.Register(K2_NEW(ctx_Editor,   CSample)(pSound->GetSound(), 0), RES_SAMPLE));
        ...
        if (pModel && pModel->GetModelFile() && pModel->GetModelFile()->GetType() == MODEL_K2)
        {
            ...
            if (pK2Model->GetNumAnims() > 0)
            {
                if (!m_pSoundModelSkeleton)
                    m_pSoundModelSkeleton = K2_NEW(ctx_Editor,   CSkeleton)();
...
{
    ResHandle hSample(g_ResourceManager.Register(K2_NEW(ctx_Editor,   CSample)(cg_worldAmbientSound, SND_2D), RES_SAMPLE));
