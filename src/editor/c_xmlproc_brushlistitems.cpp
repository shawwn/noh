...
BEGIN_XML_PROCESSOR(brushlistitems, IWidget)
    ...
    // Get the properties
    const tstring &sBrushIndex(XtoA(CBrush::GetCurrentBrushIndex()));
    const tstring &sValue(node.GetProperty(_T("value"), sBrushIndex));
    ...
    for (int i(0); i < le_loadBrushes; ++i)
    {
        ...
        // Create new listitem
        CListItem *pNewListItem(K2_NEW(ctx_Editor,   CListItem)(pInterface, pList->GetListWidget(), style));
        pNewListItem->SetValue(XtoA(i));

        // Fill new listitem
        {
            CImage *pNewImage(K2_NEW(ctx_Editor,   CImage)(pInterface, pNewListItem, style));
            ...
            if (pBrush)
            {
                CLabel *pNewLabel(K2_NEW(ctx_Editor,   CLabel)(pInterface, pNewImage, style));
                pNewLabel->SetText(XtoA(pBrush->GetBrushSize() - 2));
