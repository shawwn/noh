...
{   
    ...
    HRESULT hResult(g_pQueueLimitQuery->GetData(NULL, 0, D3DGETDATA_FLUSH));

    while (hResult == S_FALSE)
    {
        if (K2System.Milliseconds() - uiStartQuery > 500)
        {
            ...
            if (!g_DeviceCaps.bQuery)
                Console.Video << _T("Query lost") << newl;
            break;
        }

        hResult = g_pQueueLimitQuery->GetData(NULL, 0, D3DGETDATA_FLUSH);
    }

    if (hResult != S_OK)
    {
        Console.Video << _T("Query not S_OK") << newl;
