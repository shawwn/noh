...
map<tstring, CTextureFileCallback *>    g_mapTextureFileCallbacks;
...
static CTextureArchive *s_pWriteTextureArchive(NULL);
static sset s_setTextureIgnore;
bool        g_bDisableTextures(false);
//=============================================================================
...
/*====================
  CTextureFileCallback
  ====================*/
class CTextureFileCallback : public IFileChangeCallback
{
private:
...
public:
    ~CTextureFileCallback() {}
    CTextureFileCallback(const tstring &sPath, const tstring &sTexturePath) :
    ...
    void    Execute()
    {
        g_ResourceManager.Reload(m_sTexturePath);
    }
};
...
void    D3D_OpenTextureArchive(bool bNoReload)
{
    ...
    while (!sMod.empty())
    {
        g_vTextureArchives.push_back(K2_NEW(ctx_D3D9,  CTextureArchive)(sMod));
...
/*====================
  D3D_GetTextureList
  ====================*/
void    D3D_GetTextureList(const tstring &sPath, const tstring &sSearch, tsvector &vResult)
{
    for (vector<CTextureArchive*>::iterator it(g_vTextureArchives.begin()); it != g_vTextureArchives.end(); ++it)
    {
        tsvector vFiles;
...
/*====================
  D3D_WriteArchiveTexture
  ====================*/
void    D3D_WriteArchiveTexture(const tstring &sPath, IDirect3DBaseTexture9 *pTexture, int iTextureFlags)
{
    if (s_pWriteTextureArchive == NULL)
        return;

    bool bIgnored(false);
    tstring sLower(LowerString(sPath));

    for (sset::iterator itIgnore(s_setTextureIgnore.begin()), itIgnoreEnd(s_setTextureIgnore.end()); itIgnore != itIgnoreEnd; ++itIgnore)
    {
    ...
    s_pWriteTextureArchive->ActivateNode(D3D_GetTextureDefinitionString());
    s_pWriteTextureArchive->WriteTexture(sPath, pTexture, false);
}
...
IDirect3DTexture9*  D3D_CompressTexture(IDirect3DTexture9 *pOriginalTexture, D3DFORMAT eFmt, bool bMipmaps)
{
    ...
    try
    {
        ...
        if (FAILED(g_pd3dDevice->CreateTexture(desc.Width, desc.Height, uiNumLevels, desc.Usage, eFmt,
            desc.Pool, &pCompressedTexture, NULL)))
...
IDirect3DCubeTexture9*  D3D_CompressCubeTexture(IDirect3DCubeTexture9 *pOriginalTexture, D3DFORMAT eFmt, bool bMipmaps)
{
    ...
    try
    {
        ...
        // Create the new texture
        if (FAILED(g_pd3dDevice->CreateCubeTexture(desc.Width, uiNumLevels, desc.Usage, eFmt,
            desc.Pool, &pCompressedTexture, NULL)))
...
IDirect3DVolumeTexture9*    D3D_CompressVolumeTexture(IDirect3DVolumeTexture9 *pOriginalTexture, D3DFORMAT eFmt, bool bMipmaps)
{
    ...
    try
    {
        ...
        // Create the new texture
        if (FAILED(g_pd3dDevice->CreateVolumeTexture(desc.Width, desc.Height, desc.Depth, uiNumLevels, desc.Usage, eFmt,
            desc.Pool, &pCompressedTexture, NULL)))
...
/*====================
  D3D_GetSrcChannelInfo
  ====================*/
void    D3D_GetSrcChannelInfo(int iChannel, int &iStart, int &iDefault, int &iBits)
{
...
/*====================
  D3D_Register2DTextureEx
  ====================*/
int     D3D_Register2DTextureEx(const CBitmap &bitmap, int iTextureFlags, ETextureFormat eFormat, const tstring &sName, bool bCache = false, const tstring &sCachePath = TSNULL, const tstring &sCacheReference = TSNULL)
{
    try
    {
        ...
        {
            // Create the staging texture and copy into video memory
            ...
            if (FAILED(g_pd3dDevice->CreateTexture(pImage->GetWidth(), pImage->GetHeight(),
                ...
                d3dFormat, D3DPOOL_SYSTEMMEM, &pStagingTexture, NULL)))
                EX_WARN(_T("Staging CreateTexture failed"));

#if 0
            // Create the texture
            if (FAILED(g_pd3dDevice->CreateTexture(pImage->GetWidth(), pImage->GetHeight(),
                (iTextureFlags & TEX_NO_MIPMAPS) ? 1 : 0,
                g_d3dManagedUsage | ((iTextureFlags & TEX_NO_MIPMAPS) ? 0 : D3DUSAGE_AUTOGENMIPMAP),
                d3dFormat, g_d3dManagedPool, &pTexture, NULL)))
                EX_WARN(_T("CreateTexture failed"));
#endif

            // Fill the surface
            D3DLOCKED_RECT d3drect;
            if (FAILED(pStagingTexture->LockRect(0, &d3drect, NULL, 0)))
                EX_WARN(_T("LockRect failed"));

            if (d3drect.pBits)
                D3D_CopyBitmapToBuffer(*pImage, eFormat, static_cast<byte *>(d3drect.pBits), d3drect.Pitch);

            if (FAILED(pStagingTexture->UnlockRect(0)))
        ...
        else
        {
            ...
            for (uint uiLevel(0); uiLevel < pStagingTexture->GetLevelCount(); ++uiLevel)
            {
                // Fill the surface
                D3DLOCKED_RECT d3drect;
                if (FAILED(pStagingTexture->LockRect(uiLevel, &d3drect, NULL, 0)))
                    EX_WARN(_T("LockRect failed"));
                ...
                if (FAILED(pStagingTexture->UnlockRect(uiLevel)))
                    EX_WARN(_T("UnlockRect failed"));
            }
        }
        ...
        // Grayscale becomes DXT1
        if (bitmap.GetBMPType() == BITMAP_ALPHA &&
            bitmap.GetRedChannel() == 0 &&
            bitmap.GetGreenChannel() == 0 &&
            bitmap.GetBlueChannel() == 0 &&
            bitmap.GetAlphaChannel() == BMP_CHANNEL_WHITE)
        {
            eCompressedFormat = D3DFMT_DXT1;
        }
        ...
        if (vid_textureCompression && !(iTextureFlags & TEX_NO_COMPRESS) && pImage->GetWidth() >= 4 && pImage->GetHeight() >= 4)
            pStagingTexture = D3D_CompressTexture(pStagingTexture, eCompressedFormat, !(iTextureFlags & TEX_NO_MIPMAPS));
        g_llCompress = K2System.GetTicks() - g_llSectionStart;

        D3DSURFACE_DESC desc;
        pStagingTexture->GetLevelDesc(0, &desc);

        // Create the texture
        if (FAILED(g_pd3dDevice->CreateTexture(desc.Width, desc.Height, pStagingTexture->GetLevelCount(), g_d3dManagedUsage,
            desc.Format, g_d3dManagedPool, &pTexture, NULL)))
            EX_WARN(_T("CreateTexture failed"));

        g_pd3dDevice->UpdateTexture(pStagingTexture, pTexture);

        if (bCache)
        {
            if (!sCacheReference.empty())
                g_TextureCache.CacheTexture(sCachePath, pStagingTexture, sCacheReference);
            else
                g_TextureCache.CacheTexture(sCachePath, pStagingTexture);
        }

        D3D_WriteArchiveTexture(sCachePath, pStagingTexture, iTextureFlags);
    ...
    {
        ex.Process(_TS("D3D_Register2DTextureEx(") + sName + _TS(") - "), NO_THROW);
        return -1;
    }
}
...
/*====================
  ...
  ====================*/
int     D3D_Register2DTexture(const CBitmap &bitmap, int iTextureFlags, ETextureFormat eFormat, const tstring &sName, bool bCache, const tstring &sCachePath, const tstring &sCacheReference)
{
    if (g_bD3D9Ex)
        return D3D_Register2DTextureEx(bitmap, iTextureFlags, eFormat, sName, bCache, sCachePath, sCacheReference);
    ...
    try
    {
        ...
        else
        {
            ...
            // Create the texture
            if (FAILED(g_pd3dDevice->CreateTexture(bmpWorking.GetWidth(), bmpWorking.GetHeight(), 0, g_d3dManagedUsage,
                d3dFormat, g_d3dManagedPool, &pTexture, NULL)))
                EX_WARN(_T("CreateTexture failed"));
        ...
        if (vid_texturePreload && !g_bD3D9Ex)
        {
            g_llSectionStart = K2System.GetTicks();
            pTexture->PreLoad();
        ...
        {
            if (!sCacheReference.empty())
                g_TextureCache.CacheTexture(sCachePath, pTexture, sCacheReference);
            else
                g_TextureCache.CacheTexture(sCachePath, pTexture);
        }

        D3D_WriteArchiveTexture(sCachePath, pTexture, iTextureFlags);
...
int     D3D_RegisterArchivedTexture(CTexture *pTexture, const tstring &sSuffix = TSNULL)
{
    try
    {
        ...
        tstring sCallbackPath(Filename_StripExtension(sPath) + _T(".dds"));

        if (g_mapTextureFileCallbacks.find(sCallbackPath) == g_mapTextureFileCallbacks.end())
        {
            CTextureFileCallback *pTextureFileCallback(K2_NEW(ctx_D3D9,  CTextureFileCallback)(sCallbackPath, pTexture->GetPath()));
            g_mapTextureFileCallbacks[sCallbackPath] = pTextureFileCallback;

            g_ResourceManager.RegisterFileChangeCallback(pTextureFileCallback);
        }
...
CMD(WriteTextureArchive)
{
    ...
    if (pFile != NULL)
    {
        ...
        while (!pFile->IsEOF())
        {
            ...
            if (!sFilename.empty())
                s_setTextureIgnore.insert(sFilename);
        }
    }

    g_bDisableTextures = true;
    vid_textureCache = false;
    s_pWriteTextureArchive = NULL;
    ...
    g_bDisableTextures = false;

    s_pWriteTextureArchive = g_vTextureArchives.front();
    s_pWriteTextureArchive->Clear();

    // Call these explicitly to main the original order
    s_pWriteTextureArchive->WriteNode("COMPRESSION='1';DOWNSIZE='0';MIPMAPS='1';");
    s_pWriteTextureArchive->WriteNode("COMPRESSION='0';DOWNSIZE='0';MIPMAPS='1';");
    s_pWriteTextureArchive->WriteNode("COMPRESSION='0';DOWNSIZE='0';MIPMAPS='0';");
    s_pWriteTextureArchive->WriteNode("COMPRESSION='1';DOWNSIZE='1';MIPMAPS='1';");
    s_pWriteTextureArchive->WriteNode("COMPRESSION='0';DOWNSIZE='1';MIPMAPS='0';");
    s_pWriteTextureArchive->WriteNode("COMPRESSION='1';DOWNSIZE='2';MIPMAPS='1';");
    s_pWriteTextureArchive->WriteNode("COMPRESSION='0';DOWNSIZE='2';MIPMAPS='0';");
    ...
    for (int i(2); i >= 0; --i)
    {
        ...
        public:
            ...
            float   PostFrame(TextureMap::iterator &it, float f) const
            {
                ...
                {
                    s_pWriteTextureArchive->WriteTexture(Filename_AppendSuffix(it->first, _T("_rxgb")), g_pTextures[pTexture->GetIndex()], false);
                ...
                {
                    s_pWriteTextureArchive->WriteTexture(it->first, g_pTextures[pTexture->GetIndex()], false);
                }
                ++it;
        ...
        CLoadJob<TextureMap> jobWriteTextures(g_mapTextures, &fnWriteTextures, LOADING_DISPLAY_LOGO);
...
int     D3D_Register2DTextureMipmaps(const CBitmap bitmap[], int iNumLevels, int iTextureFlags, ETextureFormat eFormat, const tstring &sName)
{
    try
    {
        ...
        // Resize, if necessary
        const CBitmap **pImage = K2_NEW_ARRAY(ctx_D3D9, const CBitmap *, iNumLevels);
        ...
        if (iTextureFlags & TEX_NO_MIPMAPS)
        {
            ...
            if (FAILED(g_pd3dDevice->CreateTexture(pImage[0]->GetWidth(), pImage[0]->GetHeight(),
                1, g_d3dManagedUsage, d3dFormat, g_d3dManagedPool, &pTexture, NULL)))
                EX_WARN(_T("CreateTexture failed"));
        ...
        else
        {
            ...
            // Create the texture
            if (FAILED(g_pd3dDevice->CreateTexture(pImage[0]->GetWidth(), pImage[0]->GetHeight(), 0, g_d3dManagedUsage,
                d3dFormat, g_d3dManagedPool, &pTexture, NULL)))
                EX_WARN(_T("CreateTexture failed"));
...
int     D3D_Register2DProcedural(const CProcedural &cProcedural, int &iTexture2)
{
    try
    {
        ...
        if (cProcedural.IsMipmaps())
        {
            ...
            CBitmap *bmp = K2_NEW_ARRAY(ctx_D3D9, CBitmap, iNumLevels);
...
/*====================
  D3D_RegisterCubeTextureEx
  ====================*/
int     D3D_RegisterCubeTextureEx(const CBitmap bitmaps[], int iTextureFlags, ETextureFormat eFormat, const tstring &sName, bool bCache = false, const tstring &sCachePath = TSNULL, const tstring &sCacheReference = TSNULL)
{
    try
    {
        // Find empty texture slot
        ...
        if (vid_textureAutogenMipmaps || (iTextureFlags & TEX_NO_MIPMAPS))
        {
            ...
            if (FAILED(g_pd3dDevice->CreateCubeTexture(iNewEdgeLength,
                ...
                g_d3dManagedUsage | ((iTextureFlags & TEX_NO_MIPMAPS) ? 0 : D3DUSAGE_AUTOGENMIPMAP),
                d3dFormat, D3DPOOL_SYSTEMMEM, &pStagingTexture, NULL)))
                EX_WARN(_T("Staging CreateCubeTexture failed"));
            ...
            for (int iFace(0); iFace < 6; ++iFace)
            {
                // Fill the surface
                D3DLOCKED_RECT d3drect;
                if (FAILED(pStagingTexture->LockRect(D3DCUBEMAP_FACES(iFace), 0, &d3drect, NULL, 0)))
                    EX_WARN(_T("LockRect failed"));

                if (d3drect.pBits)
                    D3D_CopyBitmapToBuffer(*(pImage[iFace]), eFormat, static_cast<byte *>(d3drect.pBits), d3drect.Pitch);

                if (FAILED(pStagingTexture->UnlockRect(D3DCUBEMAP_FACES(iFace), 0)))
                    EX_WARN(_T("UnlockRect failed"));
            }
        }
        else
        {
            ...
            // Create the staging texture
            if (FAILED(g_pd3dDevice->CreateCubeTexture(iNewEdgeLength, 0, g_d3dManagedUsage,
                d3dFormat, D3DPOOL_SYSTEMMEM, &pStagingTexture, NULL)))
                EX_WARN(_T("Staging CreateCubeTexture failed"));
            ...
            for (int iFace(0); iFace < 6; ++iFace)
            {
                ...
                for (uint uiLevel(0); uiLevel < pStagingTexture->GetLevelCount(); ++uiLevel)
                {
                    // Fill the surface
                    D3DLOCKED_RECT d3drect;
                    if (FAILED(pStagingTexture->LockRect(D3DCUBEMAP_FACES(iFace), uiLevel, &d3drect, NULL, 0)))
                        EX_WARN(_T("LockRect failed"));
                    ...
                    if (FAILED(pStagingTexture->UnlockRect(D3DCUBEMAP_FACES(iFace), uiLevel)))
                        EX_WARN(_T("UnlockRect failed"));
                }
            }
        }
        ...
        if (vid_textureCompression && !(iTextureFlags & TEX_NO_COMPRESS) && pImage[0]->GetWidth() >= 4)
            pStagingTexture = D3D_CompressCubeTexture(pStagingTexture, eCompressedFormat, !(iTextureFlags & TEX_NO_MIPMAPS));
        g_llCompress = K2System.GetTicks() - g_llSectionStart;
        ...
        // Create the texture
        if (FAILED(g_pd3dDevice->CreateCubeTexture(desc.Width, pStagingTexture->GetLevelCount(), g_d3dManagedUsage,
            desc.Format, g_d3dManagedPool, &pTexture, NULL)))
            EX_WARN(_T("CreateCubeTexture failed"));
...
/*====================
  ...
  ====================*/
int     D3D_RegisterCubeTexture(const CBitmap bitmaps[], int iTextureFlags, ETextureFormat eFormat, const tstring &sName, bool bCache = false, const tstring &sCachePath = TSNULL, const tstring &sCacheReference = TSNULL)
{
    if (g_bD3D9Ex)
        return D3D_RegisterCubeTextureEx(bitmaps, iTextureFlags, eFormat, sName, bCache, sCachePath, sCacheReference);
...
/*====================
  D3D_RegisterVolumeTextureEx
  ====================*/
int     D3D_RegisterVolumeTextureEx(const CBitmap bitmaps[], int iDepth, int iTextureFlags, ETextureFormat eFormat, const tstring &sName, bool bCache = false, const tstring &sCachePath = TSNULL, const tstring &sCacheReference = TSNULL)
{
    const CBitmap **pImage = NULL;
    ...
    try
    {
        ...
        // Resize, if necessary
        pImage = K2_NEW_ARRAY(ctx_D3D9, const CBitmap *, iDepth);
        bmpResized = K2_NEW_ARRAY(ctx_D3D9, CBitmap, iDepth);
        ...
        if (/*vid_textureAutogenMipmaps || */(iTextureFlags & TEX_NO_MIPMAPS))
        {
            ...
            pStagingTexture->LockBox(0, &d3dbox, NULL, 0);
        ...
        else
        {
            ...
            // Create the staging texture
            if (FAILED(g_pd3dDevice->CreateVolumeTexture(iNewWidth, iNewHeight, iDepth,
                0, g_d3dManagedUsage, d3dFormat, D3DPOOL_SYSTEMMEM, &pStagingTexture, NULL)))
                EX_WARN(_T("Staging CreateVolumeTexture failed"));

            int iWorkingWidth(iNewWidth);
            int iWorkingHeight(iNewHeight);
            int iWorkingDepth(iDepth);

            CBitmap *pWorkingBitmaps(K2_NEW_ARRAY(ctx_D3D9, CBitmap, iWorkingDepth));
            ...
            {
                D3DLOCKED_BOX d3dbox;
                pStagingTexture->LockBox(uiLevel, &d3dbox, NULL, 0);
                ...
                pStagingTexture->UnlockBox(uiLevel);

                // Resize working volume texture
                CBitmap *pCopyBitmaps(K2_NEW_ARRAY(ctx_D3D9, CBitmap, iWorkingDepth));
                ...
                {
                    if (pWorkingBitmaps)
                        K2_DELETE_ARRAY(pWorkingBitmaps);

                    pWorkingBitmaps = K2_NEW_ARRAY(ctx_D3D9, CBitmap, iNewWorkingDepth);
        ...
        g_llSectionStart = K2System.GetTicks();
        if (vid_textureCompression && !(iTextureFlags & TEX_NO_COMPRESS) && pImage[0]->GetWidth() >= 4 && pImage[0]->GetHeight() >= 4)
            pStagingTexture = D3D_CompressVolumeTexture(pStagingTexture, eCompressedFormat, !(iTextureFlags & TEX_NO_MIPMAPS));
        g_llCompress = K2System.GetTicks() - g_llSectionStart;
        ...
        // Create the texture
        if (FAILED(g_pd3dDevice->CreateVolumeTexture(desc.Width, desc.Height, desc.Depth,
            pStagingTexture->GetLevelCount(), g_d3dManagedUsage, desc.Format, g_d3dManagedPool, &pTexture, NULL)))
            EX_WARN(_T("CreateVolumeTexture failed"));
    ...
    {
        if (pImage) K2_DELETE_ARRAY(pImage);
        if (bmpResized) K2_DELETE_ARRAY(bmpResized);
...
/*====================
  ...
  ====================*/
int     D3D_RegisterVolumeTexture(const CBitmap bitmaps[], int iDepth, int iTextureFlags, ETextureFormat eFormat, const tstring &sName, bool bCache = false, const tstring &sCachePath = TSNULL, const tstring &sCacheReference = TSNULL)
{
    if (g_bD3D9Ex)
        return D3D_RegisterVolumeTextureEx(bitmaps, iDepth, iTextureFlags, eFormat, sName, bCache, sCachePath, sCacheReference);
    ...
    try
    {
        ...
        else
        {
            ...
            // Create the texture
            if (FAILED(g_pd3dDevice->CreateVolumeTexture(iNewWidth, iNewHeight, iDepth,
                0, g_d3dManagedUsage, d3dFormat, g_d3dManagedPool, &pTexture, NULL)))
                EX_WARN(_T("CreateVolumeTexture failed"));
...
int     D3D_RegisterBlankTexture(const tstring &sName, int iSizeX, int iSizeY, D3DFORMAT eFormat, bool bMipmap)
{
    ...
    else
    {
        ...
        if (FAILED(g_pd3dDevice->CreateTexture(iSizeX, iSizeY, 1, g_d3dManagedUsage | dwUsage,
            eFormat, D3DPOOL_DEFAULT, &g_pTextures2D[i], NULL)))
        {
            Console.Dev << _T("D3D_RegisterRenderTargetTexture: CreateTexture failed on ") << sName << _T(" ") << iSizeX << _T("x") << iSizeY << newl;
            return -1;
...
int     D3D_RegisterDepthTexture(const tstring &sName, int iSizeX, int iSizeY, D3DFORMAT eFormat)
{
    ...
    else
    {
        ...
        if (FAILED(g_pd3dDevice->CreateTexture(iSizeX, iSizeY, 1, g_d3dManagedUsage | D3DUSAGE_DEPTHSTENCIL,
            eFormat, D3DPOOL_DEFAULT, &g_pTextures2D[i], NULL)))
        {
...
int     D3D_RegisterTexture(CTexture *pTexture)
{
    ...
    // The texture must have a path
    assert(!pTexture->GetPath().empty());
    if (pTexture->GetPath().empty())
        return 0;
    ...
    if (g_bDisableTextures)
    {
        pTexture->SetIndex(g_iWhite);

        if (pTexture->GetFormat() == TEXFMT_NORMALMAP && vid_shaderRXGBNormalmap)
            pTexture->SetIndex2(g_iBlack);
    ...
    case TEXTURE_2D:
        ...
        {
            iTexture = D3D_Register2DTexture(*pTexture->GetBitmap(), pTexture->GetTextureFlags(), pTexture->GetFormat(), pTexture->GetPath(), false);
        }
        else
        {
            ...
            if (pTexture->GetFormat() == TEXFMT_NORMALMAP && vid_shaderRXGBNormalmap)
            {
                ...
                else
                {
                    ...
                    iTexture = D3D_Register2DTexture(
                        bitmap,
                        pTexture->GetTextureFlags(),
                        TEXFMT_NORMALMAP_RXGB,
                        pTexture->GetPath(),
                        vid_textureCache,
                        Filename_AppendSuffix(pTexture->GetPath(), _T("_rxgb")),
                        pTexture->GetPath()
                    );
                }
                ...
                else
                {
                    ...
                    {
                        iTexture2 = D3D_Register2DTexture(
                            bitmap,
                            pTexture->GetTextureFlags(),
                            TEXFMT_NORMALMAP_S,
                            pTexture->GetPath(),
                            vid_textureCache,
                            Filename_AppendSuffix(pTexture->GetPath(), _T("_s")),
                            pTexture->GetPath()
                        );
                    }
                    else
                    {
                        ...
                        D3D_WriteArchiveTexture(Filename_AppendSuffix(pTexture->GetPath(), _T("_s")), g_pTextures[iTexture2], pTexture->GetTextureFlags());
                    }
                }
            ...
            else
            {
                ...
                {
                    CBitmap bitmap;
                    if (!bitmap.Load(pTexture->GetPath(), (pTexture->GetTextureFlags() & TEX_MONO_AS_ALPHA) != 0))
                    {
                        ++g_uiMissingTextures;
                        EX_WARN(_T("not found or bad texture"));
                    }
    ...
    case TEXTURE_CUBE:
        {
            ...
            else
            {
                ...
                iTexture = D3D_RegisterCubeTexture(
                    bitmaps,
                    pTexture->GetTextureFlags(),
                    pTexture->GetFormat(),
                    pTexture->GetPath(),
                    vid_textureCache,
                    pTexture->GetPath(),
                    Filename_AppendSuffix(pTexture->GetPath(), _T("_posx"))
                );
            }
        }
        break;
    ...
    case TEXTURE_VOLUME:
        {
            ...
            else
            {
                ...
                CBitmap *bitmaps(K2_NEW_ARRAY(ctx_D3D9, CBitmap, iDepth));
                ...
                iTexture = D3D_RegisterVolumeTexture(bitmaps,
                    iDepth,
                    pTexture->GetTextureFlags(),
                    pTexture->GetFormat(),
                    pTexture->GetPath(),
                    vid_textureCache,
                    pTexture->GetPath(),
                    Filename_AppendSuffix(pTexture->GetPath(), XtoA(0, FMT_PADZERO, 4))
                );
                K2_DELETE_ARRAY(bitmaps);
            }
        }
