...
HDC                     g_hDC;
IDirect3D9              *g_pD3D;
IDirect3D9Ex            *g_pD3DEx;
IDirect3DDevice9        *g_pd3dDevice;
IDirect3DDevice9Ex      *g_pd3dDeviceEx;
...
D3DPOOL                 g_d3dManagedPool;
DWORD                   g_d3dManagedUsage;
...
#if 0
CVAR_BOOLF  (d3d_modeEx,            true,   CVAR_SAVECONFIG);
#else
CVAR_BOOL   (d3d_modeEx,            false);
#endif
...
CVAR_BOOL   (win_changeDisplayMode,     true);
...
void    D3D_InitVBs()
{
    ...
    if (FAILED(g_pd3dDevice->CreateVertexBuffer(4 * sizeof(SSkyboxVertex),
        g_d3dManagedUsage | D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &g_pVBSkybox, NULL)))
...
void    D3D_InitVBsManaged()
{
    ...
    if (FAILED(g_pd3dDevice->CreateIndexBuffer(MAX_EFFECT_QUADS * 6 * sizeof(WORD),
                    g_d3dManagedUsage | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, g_d3dManagedPool, &g_pIBEffectQuad, NULL)))
    ...
    if (FAILED(g_pd3dDevice->CreateIndexBuffer(MAX_BOXES * 24 * sizeof(WORD),
                    g_d3dManagedUsage | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, g_d3dManagedPool, &g_pIBBox, NULL)))
    ...
    if (FAILED(g_pd3dDevice->CreateIndexBuffer(MAX_GUIQUADS * 6 * sizeof(WORD),
                    g_d3dManagedUsage | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, g_d3dManagedPool, &g_pIBGuiQuad, NULL)))
        K2System.Error(_T("D3D_InitVBs(): CreateIndexBuffer failed"));
    ...
    if (FAILED(g_pd3dDevice->CreateIndexBuffer(MAX_FOWQUADS * 6 * sizeof(WORD),
                    g_d3dManagedUsage | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, g_d3dManagedPool, &g_pIBFowQuad, NULL)))
        K2System.Error(_T("D3D_InitVBs(): CreateIndexBuffer failed"));
    ...
    if (FAILED(g_pd3dDevice->CreateIndexBuffer(MAX_TREE_BILLBOARDS * 6 * sizeof(WORD),
                    g_d3dManagedUsage | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, g_d3dManagedPool, &g_pIBTreeBillboard, NULL)))
        K2System.Error(_T("D3D_InitVBs(): CreateIndexBuffer failed"));
...
/*====================
  D3D_SetUsingD3D9Ex
  ====================*/
void        D3D_SetUsingD3D9Ex(bool bUsingD3D9Ex)
{
    if (bUsingD3D9Ex)
    {
        g_bD3D9Ex = true;
        g_d3dManagedPool = D3DPOOL_DEFAULT;
        g_d3dManagedUsage = 0;
    }
    else
    {
        g_bD3D9Ex = false;
        g_d3dManagedPool = D3DPOOL_MANAGED;
        g_d3dManagedUsage = 0;
    }
}
...
/*====================
  D3D_CreateD3D9
  ====================*/
HRESULT     D3D_CreateD3D9(IDirect3D9*& d3d9)
{
    HRESULT hr(E_FAIL);
    ...
    if (d3d_modeEx)
    {
        ...
        if (hD3D9)
        {
            // search for Direct3DCreate9Ex
            typedef HRESULT (WINAPI *Direct3DCreate9ExFunc)(UINT SDKVersion, IDirect3D9Ex **d3d9ex);

            Direct3DCreate9ExFunc pDirect3DCreate9Ex = (Direct3DCreate9ExFunc)GetProcAddress(hD3D9, "Direct3DCreate9Ex");
            if (pDirect3DCreate9Ex)
            {
                // use that
                hr = pDirect3DCreate9Ex(D3D_SDK_VERSION, (IDirect3D9Ex**)&d3d9);
                if (SUCCEEDED(hr))
                    D3D_SetUsingD3D9Ex(true);
            }
        }
    }

    // If d3d9 is still NULL, then just use Direct3DCreate9
    if (d3d9 == NULL)
    {
         d3d9 = Direct3DCreate9(D3D_SDK_VERSION);
         hr = S_OK;
         D3D_SetUsingD3D9Ex(false);
    }
    return hr;
}
...
void    D3D_GetDeviceCaps()
{
    ...
    ICvar::CreateInt(_T("d3d_ps_version_major"), (d3dCaps.PixelShaderVersion >> 8) & 0xff, CVAR_READONLY);
    ICvar::CreateInt(_T("d3d_ps_version_minor"), d3dCaps.PixelShaderVersion & 0xff, CVAR_READONLY);
    ICvar::CreateString(_T("d3d_ps_version"), _T("ps_") +  XtoA((d3dCaps.PixelShaderVersion >> 8) & 0xff) + _T("_") +  XtoA(d3dCaps.PixelShaderVersion & 0xff), CVAR_READONLY);

    ICvar::CreateInt(_T("d3d_vs_version_major"), (d3dCaps.VertexShaderVersion >> 8) & 0xff, CVAR_READONLY);
    ICvar::CreateInt(_T("d3d_vs_version_minor"), d3dCaps.VertexShaderVersion & 0xff, CVAR_READONLY);
    ICvar::CreateString(_T("d3d_vs_version"), _T("vs_") +  XtoA((d3dCaps.VertexShaderVersion >> 8) & 0xff) + _T("_") +  XtoA(d3dCaps.VertexShaderVersion & 0xff), CVAR_READONLY);

    d3d_vertexShaderProfile = StringToTString(D3DXGetVertexShaderProfile(g_pd3dDevice));
    d3d_pixelShaderProfile = StringToTString(D3DXGetPixelShaderProfile(g_pd3dDevice));

    Console.Video << _T("Vertex shader version: ") << ICvar::GetString(_T("d3d_vs_version")) << newl;
    Console.Video << _T("Pixel shader version: ") << ICvar::GetString(_T("d3d_ps_version")) << newl;

#if 1
    g_DeviceCaps.dwMaxPixelShaderVersion = d3dCaps.PixelShaderVersion;
    g_DeviceCaps.dwMaxVertexShaderVersion = d3dCaps.VertexShaderVersion;
#elif 0
    g_DeviceCaps.dwMaxPixelShaderVersion = D3DPS_VERSION(2, 0);
    g_DeviceCaps.dwMaxVertexShaderVersion = D3DVS_VERSION(2, 0);
#else
    g_DeviceCaps.dwMaxPixelShaderVersion = D3DPS_VERSION(1, 1);
    g_DeviceCaps.dwMaxVertexShaderVersion = D3DVS_VERSION(1, 1);
#endif

    // Determine the number of texture samplers
    ...
    case D3DPS_VERSION(1, 1):
        ...
        break;
    default:
        g_dwNumSamplers = d3dCaps.MaxSimultaneousTextures;
        break;
    }

#if 1
    if (D3DSHADER_VERSION_MAJOR(g_DeviceCaps.dwMaxPixelShaderVersion) > 0 &&
        D3DSHADER_VERSION_MAJOR(g_DeviceCaps.dwMaxVertexShaderVersion) > 0)
        g_DeviceCaps.bShaders = true;
    else
        g_DeviceCaps.bShaders = false;
#else
    g_DeviceCaps.bShaders = false;
#endif
    ...
    case D3DPS_VERSION(3, 0):
        ...
        g_DeviceCaps.bR32FShadowFilter = true;
        break;
    case D3DPS_VERSION(2, 0):
...
void    D3D_SetDefaultStates()
{
    ...
    D3D_SetVertexShaderDefault(NULL);
    D3D_SetPixelShaderDefault(NULL);
...
void    D3D_Reset()
{
    ...
    if (g_bD3D9Ex)
        Console.Video << "Using Direct3D 9 EX mode" << newl;
...
void    D3D_EndFrame(void)
{
    ...
    case D3DERR_INVALIDCALL: // Oh dear, more bad stuff
        ...
        break;
    case D3DERR_DEVICEHUNG: // TDR in 9Ex
        K2System.Error(_T("D3DERR_DEVICEHUNG"));
        break;
    case D3DERR_DEVICEREMOVED:
        K2System.Error(_T("D3DERR_DEVICEREMOVED"));
        break;
    }
...
/*====================
  D3D_Notify
  ====================*/
void    D3D_Notify(EVidNotifyMessage eMsg, int iParam1, int iParam2, int iParam3, void *pData, const tstring &sResContext)
{
    if (!g_bD3DInitialized)
        return;
...
void    D3D_GetFrameBuffer(CBitmap &bmp)
{
    ...
    // This was potentially causing crashes on some systems, so check for pSurface being null in conjunction with the LockRect() exception handling below
    if (pSurface == NULL)
        return;
    ...
    // This LockRect() call causing crashes on some systems so wrap the LockRect() in an exception
    D3DLOCKED_RECT d3dRect;
    if (FAILED(pSurface->LockRect(&d3dRect, &rect, 0)))
...
void    D3D_SetCursor(ResHandle hCursor)
{
    ...
    D3DLOCKED_RECT d3dRect;
    if (FAILED(pSurface->LockRect(&d3dRect, NULL, 0)))
        return;
