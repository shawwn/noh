...
/*====================
  ...
  ====================*/
void    CAuraDefinition::Precache(EPrecacheScheme eScheme, const tstring &sModifier)
{
    PRECACHE_GUARD
        for (tsvector_it it(m_vStateName.begin()); it != m_vStateName.end(); ++it)
            Game.Precache(*it, eScheme, sModifier);
...
/*====================
  ...
  ====================*/
void    CAuraDefinition::GetPrecacheList(EPrecacheScheme eScheme, const tstring &sModifier, HeroPrecacheList &deqPrecache)
{
    for (tsvector_it it(m_vStateName.begin()); it != m_vStateName.end(); ++it)
        Game.GetPrecacheList(*it, eScheme, sModifier, deqPrecache);
    for (tsvector_it it(m_vGadgetName.begin()); it != m_vGadgetName.end(); ++it)
        Game.GetPrecacheList(*it, eScheme, sModifier, deqPrecache);
    for (tsvector_it it(m_vReflexiveStateName.begin()); it != m_vReflexiveStateName.end(); ++it)
        Game.GetPrecacheList(*it, eScheme, sModifier, deqPrecache);
