...
public:
	...
	virtual bool		IsServerEntity() const			{ return GetServerEntity(); }
	...
	float			GetRemainingLifetimePercent() const	{ if (m_uiCreationTime != INVALID_TIME && m_uiLifetime != INVALID_TIME && m_uiLifetime != 0) return GetRemainingLifetime() / float(m_uiLifetime); else return 0.0f; }
	...
	MULTI_LEVEL_ENTITY_DEFINITION_ACCESSOR(bool, ServerEntity)
	...
	MULTI_LEVEL_ENTITY_DEFINITION_MUTATION_ACCESSOR(bool, AlwaysTransmitData)
