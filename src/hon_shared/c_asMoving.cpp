...
bool	CASMoving::ContinueStateCleanup()
{
	...
	if (m_uiMoveTime == Game.GetGameTime())
		pUnit->SetAnim(0, pUnit->GetWalkAnim(), pUnit->GetMoveSpeed() / pUnit->GetInitialMoveSpeed(), 0);
	else if (pUnit->IsPlayingAnim(0, pUnit->GetWalkAnim()))
...
bool	CASMoving::EndState(uint uiPriority)
{
	...
	if (pUnit->IsPlayingAnim(0, pUnit->GetWalkAnim()))
		pUnit->SetAnim(0, pUnit->GetIdleAnim(), 1.0f, 0);
