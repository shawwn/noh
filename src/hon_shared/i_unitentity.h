...
// MUTABLE_MULTI_LEVEL_RESOURCE_IMPL
...
protected:
	...
	uint			m_uiOrderDisjointSequence;
...
public:
	...
	virtual bool			IsValidPlayer() const		{ return (IsHero() && m_iOwnerClientNumber != -1) ? true : false; }
	...
	virtual byte		GetCharges() const				{ return 0; }
	...
	MULTI_LEVEL_BASE_ATTRIBUTE(float, HealthRegen)
	...
	virtual float	GetHealthRegen() const
	{
		...
		float fValue(GetBaseHealthRegen() * MAX(0.0f, GetHealthRegenMultiplier()) + fBonus + (fPercent * GetMaxHealth()));
	...
	MULTI_LEVEL_BASE_ATTRIBUTE(float, ManaRegen)
	...
	virtual float	GetManaRegen() const
	{
		...
		float fValue(GetBaseManaRegen() * MAX(0.0f, GetManaRegenMultiplier()) + fBonus + (fPercent * GetMaxMana()));
	...
	MULTI_LEVEL_ATTRIBUTE(float, MagicArmor, ADD, false)
	...
	virtual float	GetMoveSpeed() const
	{
		...
		float fMoveSpeed((GetBaseMoveSpeed() + fBonus) * MAX(0.0f, GetMoveSpeedMultiplier()));
	...
	MULTI_LEVEL_ATTRIBUTE(bool, AlwaysTransmitData, OR, true)
	...
	ENTITY_DEFINITION_ACCESSOR(bool, HoverOnMap)
	...
	MULTI_LEVEL_ENTITY_DEFINITION_ACCESSOR(float, UnsharedExperienceBounty)
	...
	uint					GetOrderDisjointSequence() const				{ return m_uiOrderDisjointSequence; }
	void					IncOrderDisjointSequence()						{ ++m_uiOrderDisjointSequence; }
	...
	MULTI_LEVEL_ENTITY_DEFINITION_ACCESSOR(bool, LargeUnit)

	GAME_SHARED_API virtual float		GetShield() const;
	GAME_SHARED_API virtual float		GetMaxShield() const;
	GAME_SHARED_API virtual float		GetShieldPercent() const;
};
//=============================================================================
