...
DEFINE_ENTITY_DESC(CGameInfo, 3)
{
    ...
    s_cDesc.pFieldTypes->push_back(SDataField(_T("m_yStatsStatus"), TYPE_CHAR, 3, 0));
}
//=============================================================================
...
uint    CGameInfo::GetGameOptionFromString(const tstring &sOption)
{
    ...
    else if (CompareNoCase(sOption, _T("casual")) == 0)
        return GAME_OPTION_CASUAL;
...
tstring CGameInfo::GetGameOptionName(uint uiOption)
{
    ...
    else if (uiOption == GAME_OPTION_CASUAL)                return _T("casual");
...
tstring CGameInfo::GetGameOptionsString(uint uiOptions)
{
    ...
    if ((uiOptions & GAME_OPTION_CASUAL) != 0) { if (!sReturn.empty()) sReturn += _T(", "); sReturn += Game.GetEntityString(_T("Option_Casual")); }
...
m_yStatsStatus(byte(STATS_NULL))
{
}
...
void    CGameInfo::GetSnapshot(CEntitySnapshot &snapshot, uint uiFlags) const
{
    ...
    snapshot.WriteField(m_yStatsStatus);
}
...
bool    CGameInfo::ReadSnapshot(CEntitySnapshot &snapshot, uint uiVersion)
{
    try
    {
        ...
        if (uiVersion >= 3)
        {
            snapshot.ReadField(m_yStatsStatus);
        }
