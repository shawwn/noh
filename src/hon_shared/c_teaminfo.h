...
public:
	...
	GAME_SHARED_API void			SetTeamTarget(uint uiIndex);
	uint							GetTeamTarget() const					{ return m_uiTeamTarget; }
	IUnitEntity*					GetTeamTargetAsUnit() const				{ return Game.GetUnitFromUniqueID(m_uiTeamTarget); }
	uint							GetTeamTargetTime() const				{ return m_uiTeamTargetTime; }
	void							ClearTeamTarget() 						{ m_uiTeamTarget = INVALID_INDEX; m_uiTeamTargetTime = INVALID_TIME; }
	bool							HasTeamTarget() const					{ return m_uiTeamTarget != INVALID_INDEX; }
	bool							IsTeamTarget(uint uiIndex) const		{ return m_uiTeamTarget == uiIndex; }
	bool							IsTeamTargetTimeUp() const				{ return Host.GetTime() > m_uiTeamTargetTime; }
	...
	GAME_SHARED_API float	GetWinChance() const		{ return m_fWinChance; }
