...
ResHandle	IHeroEntity::s_hMinimapTargetIcon(INVALID_RESOURCE);
...
CVAR_STRINGF(	g_heroMapTargetIcon,			"/shared/icons/minimap_ping_enemy_hero.tga",	CVAR_TRANSMIT | CVAR_GAMECONFIG);
CVAR_STRINGF(	g_heroTalkingMapIcon,			"/shared/icons/minimap_talking.tga",			CVAR_TRANSMIT | CVAR_GAMECONFIG);
CVAR_FLOATF	(	g_heroIconSizeScale,			1.25f,											CVAR_GAMECONFIG);
...
/*====================
  ...
  ====================*/
void	IHeroEntity::ClientPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme, const tstring &sModifier)
{
	...
	s_hMinimapTargetIcon = Game.RegisterIcon(g_heroMapTargetIcon);
	...
	EntityRegistry.ClientPrecache(EntityRegistry.LookupID(_T("State_Taunted")), eScheme, sModifier);
	EntityRegistry.ClientPrecache(EntityRegistry.LookupID(_T("State_Taunting")), eScheme, sModifier);
}
...
/*====================
  ...
  ====================*/
void	IHeroEntity::ServerPrecache(CEntityConfig *pConfig, EPrecacheScheme eScheme, const tstring &sModifier)
{
	IUnitEntity::ServerPrecache(pConfig, eScheme, sModifier);

	s_hLevelupEffect = Game.RegisterEffect(g_heroLevelupEffectPath);

	EntityRegistry.ServerPrecache(EntityRegistry.LookupID(_T("State_Taunted")), eScheme, sModifier);
	EntityRegistry.ServerPrecache(EntityRegistry.LookupID(_T("State_Taunting")), eScheme, sModifier);
}
...
void	IHeroEntity::Die(IUnitEntity *pAttacker, ushort unKillingObjectID)
{
	...
	if (pAttacker != NULL && pPlayerOwner != NULL && !HasUnitFlags(UNIT_FLAG_ILLUSION))
	{
		...
		if (pAttacker->IsValidPlayer())
		{
			...
			if (bSmackdown)
			{
				pAttacker->GetOwnerPlayer()->AddSmackdown();
			
				if (pKiller->GetAnnouncerVoice() != INVALID_INDEX)
				{
					CBufferFixed<2> buffer;
					buffer << GAME_CMD_SMACKDOWN_MESSAGE2 << byte(pKiller->GetAnnouncerVoice());
				...
				Game.LogAward(GAME_LOG_AWARD_SMACKDOWN, pAttacker, this);
			}
			else if (bHumilation)
			{
				pAttacker->GetOwnerPlayer()->AddHumiliation();
			
				if (pKiller->GetAnnouncerVoice() != INVALID_INDEX)
				{
					CBufferFixed<2> buffer;
					buffer << GAME_CMD_HUMILIATION_MESSAGE2 << byte(pKiller->GetAnnouncerVoice());
					Game.BroadcastGameData(buffer, true, -1, uiDelay);
				}
				else
				{	
				...
				Game.LogAward(GAME_LOG_AWARD_HUMILIATION, pAttacker, this);
			}
	...
	if (pPlayerOwner != NULL)
	{
		...
		if (pPlayerOwner->GetGold() < unGoldLoss)
			unGoldLoss = pPlayerOwner->GetGold();

		pPlayerOwner->GetGoldReport()->AddDeathGoldLost(unGoldLoss);
...
void	IHeroEntity::AssistsReward(IUnitEntity *&pKiller, CPlayer *&pPlayerKiller, ivector &vAssistPlayers)
{
	...
	for (vector<CPlayer*>::iterator it(vAssists.begin()); it != vAssists.end(); ++it)
	{
		...
		// Generate an "assist" event for the hero and each of his item slots.
		pHeroWhoAssisted->Action(ACTION_SCRIPT_ASSIST, this, pKiller);

		// Generate an "assist" event for all of their pets and their item slots.
		const uivector& vPets(pPlayerWhoAssisted->GetPets());
		for (uivector::const_iterator it(vPets.begin()), itEnd(vPets.end());
			it != itEnd;
			++it)
		{
			uint uiPetUID(*it);
			uint uiGameIndex(Game.GetGameIndexFromUniqueID(uiPetUID));
			...
			pPet->Action(ACTION_SCRIPT_ASSIST, this, pKiller);
		}
	}
	...
	if (!vAssists.empty())
	{
		ushort unAssistGoldBountyPerPlayer(ushort(INT_ROUND(GetGoldBounty() * Game.GetHeroGoldBountyAssistPercent() / vAssists.size())));
		if (unAssistGoldBountyPerPlayer > 0)
		{
			for (vector<CPlayer*>::iterator it(vAssists.begin()); it != vAssists.end(); ++it)
			{
				CPlayer* pPlayerWhoAssisted(*it);

				pPlayerWhoAssisted->GiveGold(unAssistGoldBountyPerPlayer, this, pPlayerWhoAssisted->GetHero());
				pPlayerWhoAssisted->GetGoldReport()->AddPlayerAssistGoldEarned(unAssistGoldBountyPerPlayer);

				Game.LogGold(GAME_LOG_GOLD_EARNED, pPlayerWhoAssisted, this, unAssistGoldBountyPerPlayer);
			}
		}
	}
}
...
void	IHeroEntity::FirstBloodReward(IUnitEntity *pKiller, CPlayer *pPlayerKiller)
{
	...
	pPlayerKiller->GiveGold(Game.GetHeroGoldBountyFirstBlood(), pKillerHero);
	pPlayerKiller->GetGoldReport()->AddPlayerGoldEarned(Game.GetHeroGoldBountyFirstBlood());

	if (pPlayerKiller->GetAnnouncerVoice() != INVALID_INDEX)
	{
		CBufferFixed<8> buffer;
		buffer << GAME_CMD_FIRST_BLOOD_MESSAGE2 << pPlayerKiller->GetClientNumber() << ushort(Game.GetHeroGoldBountyFirstBlood()) << byte(pPlayerKiller->GetAnnouncerVoice());
		Game.BroadcastGameData(buffer, true);	
	}
	else
	{	
		CBufferFixed<7> buffer;
		buffer << GAME_CMD_FIRST_BLOOD_MESSAGE << pPlayerKiller->GetClientNumber() << ushort(Game.GetHeroGoldBountyFirstBlood());
		Game.BroadcastGameData(buffer, true);
	}

	Game.SetFlags(GAME_FLAG_FIRST_BLOOD);
	Game.LogAward(GAME_LOG_AWARD_FIRST_BLOOD, pKiller, this, Game.GetHeroGoldBountyFirstBlood());
}
...
void	IHeroEntity::DrawOnMap(CUITrigger &minimap, CPlayer *pLocalPlayer) const
{
	...
	bool bTarget(pLocalPlayer->GetTeamInfo()->IsTeamTarget(pUnit->GetIndex()) && s_hMinimapTargetIcon != INVALID_RESOURCE);
...
/*====================
  IHeroEntity::SnapshotUpdate
  ====================*/
void	IHeroEntity::SnapshotUpdate()
{
	UpdateHeroLevel();
