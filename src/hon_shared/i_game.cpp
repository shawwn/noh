...
CVAR_UINTF( g_voteKickCooldownTime,     SecToMs(60u),   CVAR_GAMECONFIG | CVAR_TRANSMIT);
CVAR_UINTF( g_voteConcedeUnanimousTime, MinToMs(30u),   CVAR_GAMECONFIG | CVAR_TRANSMIT);
//=============================================================================


/*====================
  IGame::SetCurrentGamePointer
  ====================*/
void    IGame::SetCurrentGamePointer(IGame *pGame)
{
    ...
    {
        g_ResourceInfo.SetGameContextEnabled(true);
        if (s_pGame->IsServer())
            g_ResourceInfo.SetGameContextDivision(_T("server"));
        else
            g_ResourceInfo.SetGameContextDivision(_T("client"));
    }
}
...
/*====================
  IGame::RegisterGameMechanics
  ====================*/
ResHandle   IGame::RegisterGameMechanics(const tstring &sPath)
{
    ResHandle hRes(INVALID_RESOURCE);
    K2_WITH_GAME_RESOURCE_SCOPE()
        hRes = g_ResourceManager.Register(sPath, RES_GAME_MECHANICS);
    return RegisterGameMechanics(hRes);
}


/*====================
  IGame::FetchGameMechanics
  ====================*/
...
/*====================
  IGame::RegisterSample
  ====================*/
ResHandle   IGame::RegisterSample(const tstring &sPath)
{
    return g_ResourceManager.LookUpPath(sPath);
}


/*====================
  IGame::RegisterMaterial
  ====================*/
ResHandle   IGame::RegisterMaterial(const tstring &sPath)
{
    return g_ResourceManager.LookUpPath(sPath);
}
