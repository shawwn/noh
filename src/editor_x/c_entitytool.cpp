...
#include "../aba_shared/c_entitydefinitionresource.h"
#include "../aba_shared/i_unitdefinition.h"
    ...
    try
    {
        ...
        for (uiset::iterator it(m_setSelection.begin()); it != m_setSelection.end(); ++it)
        {
            ...
            if (findit != g_WorldEntData.end())
            {
                ...
                if (pEntity->GetType() == _T("Prop_Tree"))
...
void    CEntityTool::TreeCreate()
{
    if (m_uiHoverEnt == INVALID_INDEX)
    {
        ...
        // snap positioning 
        float fCreationX(m_v3EndPos.x);
        float fCreationY(m_v3EndPos.y);
        if (le_entitySnap)
        {
            if (le_entitySnapAbsolute)
            {
                fCreationX = ROUND(m_v3EndPos.x / le_entityPositionSnap) * le_entityPositionSnap;
                fCreationY = ROUND(m_v3EndPos.y / le_entityPositionSnap) * le_entityPositionSnap;
            }
        }

        uivector result;
        CBBoxf box = CBBoxf(-32.0f, 32.0f);
        box.Offset(CVec3f(fCreationX, fCreationY, Editor.GetWorld().GetTerrainHeight(fCreationX, fCreationY)));
        Editor.GetWorld().GetEntitiesInRegion(result, box, 0);

        for (uivector::iterator it = result.begin(); it < result.end(); ++it)
        {
            if (Editor.GetWorld().GetEntity(*it)->GetType() == _T("Prop_Tree"))
                return;
        }
        
        ResHandle hTree(g_ResourceManager.Register(le_entityTreeDefinition, RES_TREE));
        CTreeDefinitionResource *pTreeDefinition(static_cast<CTreeDefinitionResource *>(g_ResourceManager.Get(hTree)));
        le_entityModelDelta = pTreeDefinition->GetTreeModelPath();

        uint uiIndex(CreateEntity(fCreationX, fCreationY));
        if (uiIndex != INVALID_INDEX && !m_bModifier1 && !m_bModifier3)
        {
            m_setSelection.insert(uiIndex);
            EntitySelection.Trigger(XtoA(!m_setSelection.empty()));
        ...
        Editor.GetWorld().GetEntity(uiIndex)->SetTreeDefinition(hTree);

        float tmin = pTreeDefinition->GetTreeScaleMin();
        float tmax = pTreeDefinition->GetTreeScaleMax();
    ...
    else
    {
        if (m_bModifier2)
        {                           
            if (Editor.GetWorld().GetEntity(m_uiHoverEnt)->GetType() == _T("Prop_Tree"))            
            {
                if (m_setSelection.find(m_uiHoverEnt) == m_setSelection.end())
                    m_setSelection.insert(m_uiHoverEnt);
                else
