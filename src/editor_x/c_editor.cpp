...
bool	CEditor::Init(CHostClient *pHostClient)
{
	...
	try
	{
		...
		// Intialize tools
		#define REGISTER_TOOL(name) if (!ToolBox.Register(K2_NEW(global,   C##name##Tool))) EX_ERROR(_T("Failed to allocate") _T(#name) _T(" tool"))
		...
		REGISTER_TOOL(Cliff);
		REGISTER_TOOL(VisBlocker);
		#undef REGISTER_TOOL
		
		ToolBox.SelectTool(_T("deform"));

		BrushImage.Trigger(CBrush::GetCurrentBrush()->GetFilename());

		// Start with a blank world
		m_pWorld->New(_T("default"), 8, 64.0f);
	...
	for (WorldEntList_cit cit(vEntities.begin()), citEnd(vEntities.end()); cit != citEnd; ++cit)
	{
		...
		if (pModel != NULL && pModel->GetModelFile() && pModel->GetModelFile()->GetType() == MODEL_K2 && (!cEntry.bCull || !cEntry.bCullShadow))
		{
			...
			if (pK2Model->GetNumAnims() > 0)
			{
				...
				{
					findit->second.SetSkeleton(K2_NEW(global,   CSkeleton)());
				}
...
void	CEditor::UpdateMinimapTexture()
{
	...
	m_pMinimapBitmap = K2_NEW(g_heapEditor,   CBitmap)(m_pWorld->GetTileWidth(), m_pWorld->GetTileWidth(), BITMAP_RGB);

	RenderMinimapBitmap(*m_pMinimapBitmap);

	m_hMinimapTexture = g_ResourceManager.Register(K2_NEW(global,   CTexture)(_T("*editor_minimap"), m_pMinimapBitmap, TEXTURE_2D, TEX_FULL_QUALITY, TEXFMT_A8R8G8B8), RES_TEXTURE);
